...
3. `Axis3.X` / `Axis3.Z` 横向圆柱固定半径/离散半径可通过 axis-aware circle-packing grid 和保守的单支撑/同类重复多支撑/异构多支撑 supported-stack 生成候选，并进入 column generation。
4. 在最终坐标已确定的已知坐标装箱/渲染路径中，通过真实几何 guard 支持 `Axis3.X` / `Axis3.Z` 横向圆柱。
5. 横向圆柱在贴地或下方长方体支撑区间覆盖完整轴向时，可通过保守的 3D stacking 支撑检查。

## 圆柱几何语义

当前 MVP 的语义约束：

1. 固定半径和离散半径的 `Axis3.X` / `Axis3.Z` 横向圆柱可进入 axis-aware circle-packing 网格生成候选路径，以及保守的长方体 supported-stack 路径，包括单个全长支撑、同类重复多支撑和异构支撑区间覆盖完整轴向。其它候选生成、block loading、无坐标 hanging 和 pile 支撑路径仍只支持竖直圆柱（`Axis3.Y`），除非某条路径另行明确说明。
...
| `Axis3.X` | 横向圆柱，轴向沿 X；截面圆位于 YZ 平面。 | 支持固定/离散半径的 axis-aware circle-packing 网格候选、保守的单支撑、同类重复多支撑或异构多支撑轴向覆盖 generated supported-stack 候选、已知坐标最终装箱/渲染路径，以及贴地/长方体支撑覆盖下的 3D stacking 检查；无坐标 hanging 和 cuboid-only 生成路径仍不支持。 |
| `Axis3.Z` | 横向圆柱，轴向沿 Z；截面圆位于 XY 平面。 | 支持固定/离散半径的 axis-aware circle-packing 网格候选、保守的单支撑、同类重复多支撑或异构多支撑轴向覆盖 generated supported-stack 候选、已知坐标最终装箱/渲染路径，以及贴地/长方体支撑覆盖下的 3D stacking 检查；无坐标 hanging 和 cuboid-only 生成路径仍不支持。 |
...
| layer generation fallback / circle packing / pile | 支持 | 支持竖直圆柱候选；pile 支撑仅限直立 `Axis3.Y` 圆柱 | circle packing 支持固定/离散半径的 axis-aware 横向网格候选和保守的单支撑/同类重复多支撑/异构多支撑轴向覆盖 supported-stack 候选；fallback 和 pile 仍不支持 |
...
| stacking / hanging 支撑语义 | 支持长方体语义 | 仅在明确 guard 的直立 `Axis3.Y` 支撑检查中受限支持 | 3D stacking 支持贴地或长方体支撑区间覆盖完整轴向；无坐标 hanging、径向局部支撑和底部圆柱支撑仍不支持 |
