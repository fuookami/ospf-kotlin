...
| 包 | 描述 | 关键类型 |
|---|------|---------|
| `dimension` | 物理量纲和物理量 | `FundamentalQuantity`、`DerivedQuantity`、`Dimensions` |
| `quantity` | 物理量类型和运算 | `Quantity<V>`、`DurationExtensions` |
| `math/symbol` | 带物理量语义的符号辅助类型 | `SymbolQuantity`、`DimensionedSymbol`、`SymbolDimensionRegistry` |
...
```
FundamentalQuantity（SI 基本物理量）
├── Length（长度）、Mass（质量）、Time（时间）、Current（电流）
├── Temperature（温度）、Amount（物质的量）、LuminousIntensity（发光强度）
