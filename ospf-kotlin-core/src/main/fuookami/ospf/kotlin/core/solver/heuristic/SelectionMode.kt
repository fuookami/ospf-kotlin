/**
 * 选择模式接口与实现 / Selection mode interface and implementations
*/
...
/**
 * 选择模式接口，定义如何从种群中选择个体数量。 / Selection mode interface, defining how to select the number of individuals from the population.
 *
...
interface SelectionMode<ObjValue, V> where V : RealNumber<V>, V : NumberField<V> {
    ...
    /**
     * 计算种群中应选择的个体数量。 / Calculate the number of individuals to select from the population.
     *
...
/**
 * 静态选择模式，使用密度范围下界作为选择数量。 / Static selection mode, using density range lower bound as selection count.
 *
 * @param ObjValue 目标值类型 / Objective value type
 * @param V 值类型 / Value type
*/
...
/**
 * 自适应动态选择模式，根据适应度和密度动态调整选择数量。 / Adaptive dynamic selection mode, dynamically adjusting selection count based on fitness and density.
*/
