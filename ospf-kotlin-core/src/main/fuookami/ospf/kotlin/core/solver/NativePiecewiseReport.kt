...
private fun restoreValues(
    ...
    for (structure in semiStructures) {
        ...
        val lower = structure.converter.fromValue(structure.lowerBound).toSolverDouble("semi.lowerBound")
        semiHelpers[structure.indicatorVariable.key] = if (result.toSolverDouble("semi.result") >= lower) Flt64.one else Flt64.zero
    }
...
private fun nativeFingerprint(
    ...
    for (structure in semis) {
        ...
        val lower = structure.converter.fromValue(structure.lowerBound).toSolverDouble("semi.lowerBound")
        val upper = structure.converter.fromValue(structure.upperBound).toSolverDouble("semi.upperBound")
...
public fun restoreNativePiecewiseSolution(
    ...
    for (structure in normalizedIndicator) {
        ...
        val flag = solution.values.getOrNull(resultIndex)?.toSolverDouble("indicator.result") ?: return failure("Indicator result value missing")
        var input = structure.constant
        for ((key, coefficient) in structure.terms) {
            val column = columns[key] ?: return failure("Indicator input column missing")
                input += coefficient * (solution.values.getOrNull(column)?.toSolverDouble("indicator.input") ?: return failure("Indicator input value missing"))
        }
        ...
        structure.difference?.let { combined ->
            val secondColumn = columns[combined.condition.resultKey] ?: return failure("Difference condition column missing")
            val outputColumn = columns[combined.resultKey] ?: return failure("Difference result column missing")
            val second = solution.values.getOrNull(secondColumn)?.toSolverDouble("difference.condition") ?: return failure("Difference condition value missing")
            val output = solution.values.getOrNull(outputColumn)?.toSolverDouble("difference.result") ?: return failure("Difference result value missing")
        ...
        structure.conjunction?.let { combined ->
            val secondColumn = columns[combined.condition.resultKey] ?: return failure("Conjoined condition column missing")
            val outputColumn = columns[combined.resultKey] ?: return failure("Conjunction result column missing")
            val second = solution.values.getOrNull(secondColumn)?.toSolverDouble("conjunction.condition") ?: return failure("Conjoined condition value missing")
            val output = solution.values.getOrNull(outputColumn)?.toSolverDouble("conjunction.result") ?: return failure("Conjunction result value missing")
        ...
        val measuredInput = if (band == null) input else {
            val sideColumn = columns[band.sideKey] ?: return failure("Zero-band side column missing")
            val side = solution.values.getOrNull(sideColumn)?.toSolverDouble("zeroBand.side") ?: return failure("Zero-band side value missing")
        ...
        for (key in structure.equivalentResultKeys) {
            ...
            val equivalent = solution.values.getOrNull(column)?.toSolverDouble("indicator.equivalent")
        ...
        structure.impliedCondition?.let { value ->
            val consequentColumn = columns[value.resultKey] ?: return failure("Implied indicator column missing")
            val consequentFlag = solution.values.getOrNull(consequentColumn)?.toSolverDouble("impliedCondition.result") ?: return failure("Implied indicator value missing")
            ...
            for ((key, coefficient) in value.terms) {
                ...
                consequentInput += coefficient * (solution.values.getOrNull(column)?.toSolverDouble("impliedCondition.input") ?: return failure("Implied input value missing"))
            }
        ...
        structure.conditionalValue?.let { value ->
            val outputColumn = columns[value.resultKey] ?: return failure("Conditional value result column missing")
            val output = solution.values.getOrNull(outputColumn)?.toSolverDouble("conditionalValue.result") ?: return failure("Conditional value result missing")
            var trueValue = value.constant
            for ((key, coefficient) in value.terms) {
                val column = columns[key] ?: return failure("Conditional value input column missing")
                trueValue += coefficient * (solution.values.getOrNull(column)?.toSolverDouble("conditionalValue.input") ?: return failure("Conditional value input missing"))
            }
    ...
    for (structure in normalizedMasking) {
        ...
        val mask = solution.values.getOrNull(maskColumn)?.toSolverDouble("masking.mask") ?: return failure("Mask value missing")
        val output = solution.values.getOrNull(outputColumn)?.toSolverDouble("masking.result") ?: return failure("Masking result missing")
        ...
        for ((key, coefficient) in structure.value.terms) {
            ...
            input += coefficient * (solution.values.getOrNull(column)?.toSolverDouble("masking.input") ?: return failure("Masking input value missing"))
        }
        ...
        structure.definition?.let { definition ->
            ...
            for ((key, coefficient) in definition.terms) {
                ...
                definedMask += coefficient * (solution.values.getOrNull(column)?.toSolverDouble("masking.definitionInput") ?: return failure("Mask definition input missing"))
            }
