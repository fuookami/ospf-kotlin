...
/**
 * 约束关系枚举，表示小于等于、等于、大于等于三种约束方向。 / Constraint relation enumeration representing three constraint directions: less-equal, equal, greater-equal.
*/
enum class ConstraintRelation {
    /** 小于等于 / Less than or equal */
    LessEqual {
        override val reverse get() = GreaterEqual
        override fun <T : Ord<T>> operator(): Comparator<T> = Comparator { lhs, rhs -> lhs.compareTo(rhs) <= 0 }
    ...
    /**
     * 获取此约束关系的反转关系（如小于等于变为大于等于）。 / Get the reverse of this constraint relation (e.g. less-equal becomes greater-equal).
     *
    ...
    */
    ...
    /**
     * 获取此约束关系对应的比较器。 / Get the comparator corresponding to this constraint relation.
     *
    ...
    */
    ...
    /**
     * 直接判断两个值是否满足此约束关系。 / Directly check whether two values satisfy this constraint relation.
     *
     * @param T 可比较的有序类型 / An ordered type
    ...
    */
    ...
    /**
     * 将此约束关系转换为通用比较枚举。 / Convert this constraint relation to the generic comparison enumeration.
     *
