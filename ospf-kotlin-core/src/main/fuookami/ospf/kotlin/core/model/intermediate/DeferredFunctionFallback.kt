...
/**
 * 延迟函数 fallback 生成的求解器无关约束。 / Solver-neutral constraints generated by deferred-function fallback.
 *
 * @property constraints Flt64 约束列表 / Flt64 constraint list
 */
...
/**
 * 将延迟函数结构物化为 fallback 约束。 / Materialize a deferred-function structure into fallback constraints.
 */
fun interface DeferredFunctionFallbackMaterializer {
    /**
     * 物化指定结构。 / Materialize the specified structure.
     *
     * @param structure 待物化的结构 / Structure to materialize
     * @return 物化后的约束或错误 / Materialized constraints or an error
     */
    fun materialize(structure: DeferredFunctionStructure): Ret<DeferredFunctionFallbackConstraints>
}

/**
 * 接收 fallback 约束并支持事务回滚的目标。 / Target that accepts fallback constraints and supports transaction rollback.
 */
interface DeferredFunctionFallbackTarget {
    ...
    /**
     * 追加约束。 / Append constraints.
     *
     * @param constraints 待追加的约束 / Constraints to append
     * @return 追加结果 / Append result
     */
    fun append(constraints: List<LinearInequality<Flt64>>): Try

    /**
     * 回滚到指定行数。 / Roll back to the specified row count.
     *
     * @param constraintCount 目标约束行数 / Target constraint count
...
/**
 * 事务化物化一组延迟函数 fallback。 / Materialize a group of deferred-function fallbacks transactionally.
 *
 * @param structures 待物化的结构 / Structures to materialize
 * @param target 接收约束的目标 / Constraint target
 * @param materializer 约束物化器 / Constraint materializer
 * @return 每个结构对应的约束区域或错误 / Constraint region for each structure or an error
 */
...
private fun rollbackErrors(
    primaryErrors: List<Error<ErrorCode>>,
    rollback: Try
