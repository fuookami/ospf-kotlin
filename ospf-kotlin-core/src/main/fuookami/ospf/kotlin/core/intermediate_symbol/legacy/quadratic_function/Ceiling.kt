...
@Deprecated(
    message = "Use intermediate_symbol.function.CeilingFunction instead. The linear version handles quadratic inputs.",
    replaceWith = ReplaceWith("CeilingFunction", "fuookami.ospf.kotlin.core.intermediate_symbol.function.CeilingFunction")
)
class CeilingFunction(
    ...
    private val possibleRange: ValueRange<Flt64>
        get() {
            return if (d.range.range!!.contains(Flt64.zero)) {
                ...
                val q1 = (x.upperBound!!.value.unwrap() / d.upperBound!!.value.unwrap()).ceil()
                val q2 = (x.upperBound!!.value.unwrap() / d.lowerBound!!.value.unwrap()).ceil()
                val q3 = (x.lowerBound!!.value.unwrap() / d.upperBound!!.value.unwrap()).ceil()
                val q4 = (x.lowerBound!!.value.unwrap() / d.lowerBound!!.value.unwrap()).ceil()
    ...
    override fun prepare(values: Map<Symbol, Flt64>?, tokenTable: AbstractTokenTable): Flt64? {
        return prepareIfNotCached(values, tokenTable) {
            ...
            val rValue = dValue * qValue - xValue
