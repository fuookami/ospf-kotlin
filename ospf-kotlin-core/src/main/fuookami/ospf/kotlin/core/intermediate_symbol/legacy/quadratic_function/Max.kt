    ...
    override fun register(model: AbstractQuadraticMechanismModel): Try {
        for ((i, polynomial) in polynomials.withIndex()) {
            when (val result = model.addConstraint(
                relation = minmax geq polynomial,
        ...
        if (exact) {
            for ((i, polynomial) in polynomials.withIndex()) {
                when (val result = model.addConstraint(
                    relation = minmax leq (polynomial + m * (Flt64.one - u[i])),
    ...
    override fun register(
        ...
        if (exact) {
            for ((i, polynomial) in polynomials.withIndex()) {
                if (i == index) {
                    when (val result = model.addConstraint(
                        relation = minmax leq polynomial,
                        name = "${name}_ub_${polynomial.name.ifEmpty { "$i" }}",
                        from = parent ?: this
                    )) {
                        is Ok -> {}
                    ...
                    when (val result = model.addConstraint(
                        relation = minmax leq (polynomial + m),
                        name = "${name}_ub_${polynomial.name.ifEmpty { "$i" }}",
                        from = parent ?: this
                    )) {
                        is Ok -> {}
...
@Deprecated(
    message = "Use intermediate_symbol.function.MaxFunction instead. The linear version handles quadratic inputs.",
