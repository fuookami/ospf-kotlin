    ...
    override suspend fun prepare(tokenTable: AbstractTokenTable) {
        ...
        if (tokenTable.tokenList.tokens.any { it.result != null }) {
            x.value(tokenTable)?.let { xValue ->
                y.value(tokenTable)?.let { yValue ->
                    if (_neg != null) {
                        val value = max(Flt64.zero, yValue - xValue)
                        logger.trace { "Setting SlackFunction ${name}.neg initial solution: $value" }
                        tokenTable.find(_neg!!)?.let { it._result = value }
                    }
                    if (_pos != null) {
                        val value = max(Flt64.zero, xValue - yValue)
                        logger.trace { "Setting SlackFunction ${name}.pos initial solution: $value" }
