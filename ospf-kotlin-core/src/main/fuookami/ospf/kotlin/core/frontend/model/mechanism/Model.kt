...
typealias AbstractQuadraticModel = Model<QuadraticMonomialCell, Quadratic>
typealias AbstractSingleObjectQuadraticModel = SingleObjectModel<QuadraticMonomialCell, Quadratic>

class QuadraticModel(
    private val parent: QuadraticMetaModel,
    override var name: String,
    private val _constraints: MutableList<QuadraticConstraint>,
    override val objectFunction: SingleObject<Quadratic>,
    override val tokens: QuadraticTokenTable
) : AbstractSingleObjectQuadraticModel {
    companion object {
        suspend operator fun invoke(metaModel: QuadraticMetaModel): Ret<QuadraticModel> {
            val tokens = when (val result = unfold(metaModel.tokens)) {
                is Ok -> {
                    result.value
                }
            ...
            for (symbol in tokens.symbols) {
                if (symbol is QuadraticFunctionSymbol) {
                    symbol.register(model)
                }
            }
        ...
        private suspend fun unfold(tokens: QuadraticMutableTokenTable): Ret<QuadraticTokenTable> {
            val temp = tokens.copy()
            for (symbol in temp.symbols) {
                if (symbol is QuadraticFunctionSymbol) {
                    when (val result = symbol.register(temp)) {
    ...
    override fun addConstraint(constraint: Inequality<*, QuadraticMonomialCell, Quadratic>, name: String) {
        constraint.name = name
        constraints.add(QuadraticConstraint(constraint, tokens))
    }
}

class MultiObjectQuadraticModel {}
