...
internal fun LinearInequality.register(
    ...
    if (rhs.constant in lhs.range.valueRange!!) {
        when (val result = model.addConstraint(
            relation = lhs leq (k[0] * lhs.lowerBound!!.value.unwrap() + k[1] * rhs.constant + k[2] * lhs.upperBound!!.value.unwrap()),
        ...
        when (val result = model.addConstraint(
            relation = lhs geq (k[0] * lhs.lowerBound!!.value.unwrap() + k[1] * rhs.constant + k[2] * lhs.upperBound!!.value.unwrap()),
            name = "${name.ifEmpty { parentName }}_ub",
            from = parent
        )) {
            is Ok -> {}
