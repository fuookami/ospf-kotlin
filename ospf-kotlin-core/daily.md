...
状态：已完成（完成 key-path `TODO("not implemented yet")` 语义化、core 乱码注释清理、CodeHeap 文档增强）。
...
1. 清理 `mosek` 插件主路径 `TODO("not implemented yet")`：
   - `ospf-kotlin-core-plugin/ospf-kotlin-core-plugin-mosek/src/main/.../MosekSolver.kt`
   - `ospf-kotlin-core-plugin/ospf-kotlin-core-plugin-mosek/src/main/.../MosekLinearSolver.kt`
   - 将 `TODO` 改为 `Failed(Err(...))`，统一接入现有错误码体系并提供中英双语提示信息，避免默认 `NotImplementedError` 缺少上下文。
2. 修复 core 主链路明显乱码注释（仅注释，不改逻辑）：
   - `LinearTriadModel.kt`
   - `QuadraticTetradModel.kt`
   - `BasicMechanismModel.kt`
   - `MathInequalityDsl.kt`
   - `MechanismModel.kt`
   - `TokenTable.kt`
3. 改善 CodeHeap 告警文档入口（双语同步）：
   - `README.md`
   - `README_ch.md`
   - 在原有 `MAVEN_OPTS` 建议基础上补充可直接执行的 `PowerShell` 当前会话、`Windows setx` 持久化、`bash/zsh export` 三类命令。
4. 扫描结论（残余风险记录）：
   - `mosek` 主路径 `TODO("not implemented yet")` 已清零。
   - `core/src/main` 的乱码字符扫描已清零。
   - `example/framework` 下仍有大量历史 `TODO("not implemented yet")` 与乱码文本，属于非默认阻塞路径，本批未做行为性补齐，仅记录为后续清理项。

本批验收命令与结果：

1. `mvn --% -pl ospf-kotlin-core-plugin/ospf-kotlin-core-plugin-mosek -am -DskipTests compile`：通过。
2. `mvn --% -pl ospf-kotlin-core -am -DskipTests test-compile`：通过。
3. `pwsh.exe -NoProfile -ExecutionPolicy Bypass -File .\ospf-kotlin-core\scripts\check-c8-guards.ps1 -GuardMode P6`：通过。
4. `pwsh.exe -NoProfile -ExecutionPolicy Bypass -File .\ospf-kotlin-core\scripts\check-c8-guards.ps1 -GuardMode P7`：通过。
5. `git diff --check`：通过（仅换行风格提示，无格式错误）。
6. `git grep -n 'TODO("not implemented yet")' -- ospf-kotlin-core-plugin/ospf-kotlin-core-plugin-mosek/src/main/*.kt`：无结果（通过）。
7. `Select-String -Pattern '�'` 扫描 `ospf-kotlin-core/src/main`：无结果（通过）。
