...
P26 已完成，P26-1 到 P26-4 已闭环；P27 草案已创建，P27-1 已先行完成，建议继续按低风险工程健康批次推进。P27 不重新打开 P18-P26 的迁移目标，不恢复旧兼容层，不把 benchmark 数值设为硬门禁。
...
1. P27-3：先增强 benchmark trend smoke 断言，风险低且验证快。
2. P27-2：随后评估 heuristic cleanup 的异常路径语义，避免和算法逻辑混杂。
3. P27-4：最后处理测试侧 converter 样板，按实际差异决定是否提取 helper。
