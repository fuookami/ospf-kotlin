...
4. `check-c8-guards.ps1` 的 `P6` 与 `P7` 模式（含 P10/P11/P12/P14/P16 静态门禁）。
5. P16 恢复检查：`ospf-kotlin-example/src` 下不再存在 `non-default-main` / `non-default-test`，且默认源码集不允许旧 `core.frontend.*` / `core.backend.*` / `utils.math.*` import 回流。
