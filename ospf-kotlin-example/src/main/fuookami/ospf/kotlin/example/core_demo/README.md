...
`core_demo` contains runnable optimization model construction demos that showcase the OSPF Kotlin core APIs. Each demo builds a linear or quadratic meta-model, registers variables/symbols/constraints/objectives, solves it, and extracts the solution. `GenericNumberDemo` verifies that the model builders work correctly across multiple numeric types (`Flt64`, `Rtn64`, `FltX`, `RtnX`).
