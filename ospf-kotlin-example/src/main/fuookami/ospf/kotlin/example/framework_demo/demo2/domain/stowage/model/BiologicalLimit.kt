 ...
 * 或哪些类型完全禁止散货装载。
 *
 * @property adjacentLimit 不能放在相邻位置的货物类型对 / pairs of cargo types that must not be placed in adjacent positions
 * @property bulkConflictLimit 不能共享同一散货位置的货物类型对 / pairs of cargo types that must not share the same bulk position
 * @property bulkLimit 被限制进行散货装载的货物类型 / cargo types that are restricted from bulk stowage
*/
