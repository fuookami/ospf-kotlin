...
class HorizontalStabilizer(
    ...
    /**
     * 根据起飞重量查找最接近的配平值。 / Returns the trim value nearest to the given takeoff weight.
     *
     * @param tow 用于查找的起飞重量 / Takeoff weight used for the lookup
     * @param mac 此调用关联的 MAC 值（当前查找按重量进行） / MAC value associated with this call (the current lookup uses weight)
     * @return 匹配点的配平值，找不到时返回零 / Trim value of the matching point, or zero when no point exists
     */
