...
sealed class SolveMode {
    ...
    */
     ...
     *
     * @property config 有效的 Benders 自适应配置 / The effective Benders adaptive configuration.
    */
 ...
 *
 * @property minBinaryVariables 启用 Benders 所需的最小二值变量数 / Minimum number of binary variables required to enable Benders.
 * @property maxIterations Maximum number of Benders iterations. / Benders 最大迭代次数
 * @property tolerance 主子问题目标间隙的收敛容差 / Convergence tolerance for the gap between master and sub objectives.
 * @property maxStallIterations 终止前允许的最大连续无新增割迭代数 / Maximum number of consecutive iterations without a new cut before termination.
 * @property objectiveStallIterations 最大连续目标停滞迭代数 / Maximum number of consecutive objective-stalled iterations.
*/
 ...
 *
 * @property weakGapMultiplier 应用于容差的乘数，用于确定弱间隙阈值 / Multiplier applied to tolerance to determine weak gap threshold.
 * @property weakGapFloor 弱间隙阈值的下限值 / Floor value for the weak gap threshold.
 * @property iterationPressurePercent 触发迭代压力的最大迭代百分比 / Percentage of max iterations at which iteration pressure triggers.
 * @property cutDensityMinIterations 应用割密度检查前的最小迭代次数 / Minimum iterations before cut density check is applied.
 * @property cutDensityThreshold 低于此阈值的割密度被视为低 / Threshold below which cut density is considered low.
 * @property trajectoryMinSnapshots 轨迹分析所需的最小快照数 / Minimum snapshots required for trajectory analysis.
 * @property trajectoryStepMultiplier 应用于容差的轨迹步长阈值乘数 / Multiplier applied to tolerance for trajectory step threshold.
 * @property trajectoryStepFloor 轨迹步长阈值的下限值 / Floor value for the trajectory step threshold.
 * @property timeGuardMinMs 时间守卫触发前的最小毫秒数 / Minimum time in milliseconds before time guard can trigger.
 * @property scoreGapWeight 间隙风险在质量评分中的权重 / Weight of gap risk in quality score.
 * @property scoreTimeWeight 时间风险在质量评分中的权重 / Weight of time risk in quality score.
 * @property scoreIterationWeight 迭代风险在质量评分中的权重 / Weight of iteration risk in quality score.
 * @property scoreCutDensityWeight 割密度风险在质量评分中的权重 / Weight of cut density risk in quality score.
 * @property scoreTrajectoryWeight 轨迹风险在质量评分中的权重 / Weight of trajectory risk in quality score.
*/
...
object BendersStrategy {
     ...
     *
     * @param aircraftType 要检查的航空器类型输入 / The aircraft type input to check.
     * @return 如果航空器类型受支持则返回 true，否则返回 false / True if the aircraft type is supported, false otherwise.
    */
     ...
     *
     * @param configured 用户配置的 Benders 自适应配置 / The user-configured Benders adaptive config.
     * @param binaryVariables 问题中的二值变量数量 / The number of binary variables in the problem.
     * @return 调优后的有效 Benders 自适应配置 / The tuned effective Benders adaptive configuration.
    */
     ...
     *
     * @param request 包含求解策略和问题数据的请求 DTO / The request DTO containing solve policy and problem data.
     * @param notes 收集诊断信息的可变列表 / Mutable list for collecting diagnostic notes.
     * @return 解析后的求解模式 / The resolved solve mode.
    */
     ...
     *
     * @param override 用户提供的质量覆盖配置，null 表示使用默认值 / User-provided quality override configuration, or null for defaults.
     * @return 归一化权重后的质量守卫配置 / The resolved quality guard configuration with normalized weights.
    */
     ...
     * 根据自适应配置容差解析间隙守卫阈值。
     *
     * @param adaptive 有效的 Benders 自适应配置 / The effective Benders adaptive configuration.
     * @return 间隙守卫阈值 / The gap guard threshold value.
    */
     ...
     *
     * @param adaptive 有效的 Benders 自适应配置 / The effective Benders adaptive configuration.
     * @param qualityGuard 质量守卫配置 / The quality guard configuration.
     * @return 时间守卫阈值（毫秒） / The time guard threshold in milliseconds.
    */
     ...
     * @param adaptive 有效的 Benders 自适应配置 / The effective Benders adaptive configuration.
     * @param qualityGuard 质量守卫配置 / The quality guard configuration.
     * @param bendersIterations 执行的 Benders 迭代次数 / The number of Benders iterations performed.
     * @param bendersGap 最终 Benders 间隙 / The final Benders gap.
     * @param bendersTimeMs The total Benders solve time in milliseconds. / Benders 总求解时间（毫秒）
     * @param executedIterations 实际执行的迭代次数，如果与 bendersIterations 不同 / The actual number of iterations executed, if different from bendersIterations.
     * @param totalCuts 生成的割总数 / The total number of cuts generated.
     * @param iterationSnapshots 每次迭代的主问题目标值列表，用于轨迹分析 / List of master objective values per iteration for trajectory analysis.
     * @return 质量不足时的原因字符串，可接受时返回 null / A quality reason string if quality is insufficient, or null if acceptable.
    */
     ...
     * @param bendersGap 最终 Benders 间隙 / The final Benders gap.
     * @param bendersTimeMs The total Benders solve time in milliseconds. / Benders 总求解时间（毫秒）
     * @param executedIterations 实际执行的迭代次数 / The actual number of iterations executed, if different from bendersIterations.
     * @param totalCuts 生成的割总数 / The total number of cuts generated.
     * @param iterationSnapshots 每次迭代的主问题目标值列表 / List of master objective values per iteration for trajectory analysis.
