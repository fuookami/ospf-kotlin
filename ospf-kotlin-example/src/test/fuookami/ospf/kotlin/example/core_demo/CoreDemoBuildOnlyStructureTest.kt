...
class CoreDemoBuildOnlyStructureTest {
    ...
    fun demo3ShouldBuildLinearMetaModelWithCorrectStructure() {
        ...
        try {
            ...
            val yield_ = LinearIntermediateSymbols("yield", Shape1(d3Products.size))
            for (p in d3Products) {
    ...
    fun demo4ShouldBuildLinearMetaModelWithCorrectStructure() {
        ...
        try {
            ...
            model.add(profit)
            val use = LinearIntermediateSymbols("use", Shape1(d4Materials.size))
    ...
    fun demo7ShouldBuildLinearMetaModelWithCorrectStructure() {
        ...
        try {
            ...
            val shipment = LinearIntermediateSymbols("shipment", Shape1(d7Warehouses.size))
            for (w in d7Warehouses) {
                shipment[w].asMutable() += sum(d7Stores.filter { w.cost.contains(it) }.map { s -> x[w, s] })
            }
            model.add(shipment)

            val purchase = LinearIntermediateSymbols("purchase", Shape1(d7Stores.size))
            for (s in d7Stores) {
