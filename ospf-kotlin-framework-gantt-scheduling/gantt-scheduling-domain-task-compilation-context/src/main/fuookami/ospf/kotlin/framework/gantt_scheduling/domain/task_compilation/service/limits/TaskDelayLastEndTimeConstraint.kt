...
class TaskDelayLastEndTimeConstraint<
    ...
    /**
     * 通过 solver 时间窗口边界创建任务延迟最晚结束时间约束 / / Create task delay last end time constraint from a solver time-window boundary
     *
    ...
    */
    constructor(
        ...
        name: String = "task_delay_last_end_time"
    ) : this(
    ...
    override operator fun invoke(model: AbstractLinearMetaModel<Flt64>): Try {
        for (task in tasks) {
            ...
            when (val result = model.addConstraint(
                taskTime.estimateEndTime[task] leq timeBoundary.valueOf(lastEndTime),
                name = "${name}_${task}",
