...
suspend inline fun <T> List<T>.findImpl(
    time: TimeRange,
...
suspend inline fun <T> List<T>.findFromImpl(
    time: Instant,
    crossinline extractor: Extractor<TimeRange, T>
): Pair<Int, Int>? {
...
suspend inline fun <T> List<T>.findUntilImpl(
    time: Instant,
    crossinline extractor: Extractor<TimeRange, T>
): Pair<Int, Int>? {
    return this.findImpl(
...
suspend fun <T> List<T>.findUntil(
    time: Instant,
    extractor: KProperty1<T, TimeRange>
): List<T> {
...
suspend inline fun <T> List<T>.find(
    time: TimeRange,
    crossinline extractor: Extractor<TimeRange, T>
): List<T> {
...
suspend inline fun <T> List<T>.findFrom(
    time: Instant,
    crossinline extractor: Extractor<TimeRange, T>
): List<T> {
    val timeRanges = this.map { extractor(it) }
...
suspend inline fun <T> List<T>.findUntil(
    time: Instant,
    crossinline extractor: Extractor<TimeRange, T>
): List<T> {
    val timeRanges = this.map { extractor(it) }
