 ...
 * 中文UUIDv7 是一种时间排序的 UUID 格式，将 Unix 时间戳与随机位组合，提供适合分布式系统的可排序唯一标识符。
 */
data object UUIDv7 {
     ...
     * 中文UUID 结构：字节 0-5：48 位 Unix 时间戳（毫秒）；字节 6-7：版本（4 位）+ 随机数据（12 位）；字节 8-15：变体（2 位）+ 随机数据（62 位）
     *
     ...
     * 中文同步生成一个新的 UUIDv7 作为 16 字节数组。此方法是线程安全的，可在并发环境中使用。
     *
     * @return a 16-byte array representing the UUIDv7 / 16 字节的 UUIDv7 字节数组
     */
    @Synchronized
