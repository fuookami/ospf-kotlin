/**
 * 系统工具函数 / System utility functions for memory management and monitoring.
 * 用于内存管理和监控的系统工具函数。
*/
package fuookami.ospf.kotlin.utils

/**
 * 检查内存使用是否超过阈值 / Checks if the current memory usage exceeds the specified threshold.
 * 检查当前内存使用是否超过指定的阈值。 / This function calculates the used memory as (total memory - free memory)
