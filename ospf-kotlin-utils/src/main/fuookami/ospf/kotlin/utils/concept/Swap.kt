/**
 * 交换概念接口 / Swap concept interface providing swap semantics for types.
 * 为类型提供交换语义的交换概念接口。 / This interface defines a swap operation that exchanges the internal state
...
/**
 * 可交换接口 / Interface for types that support swap semantics.
 * 支持交换语义的类型接口。 / Types implementing this interface can exchange their internal state
...
interface Swappable<Self> {
    ...
    /**
     * 与另一个实例交换状态 / Swaps the internal state with another instance.
     * 与另一个实例交换内部状态。
     *
...
/**
 * 交换两个元素的便捷函数 / Convenience function to swap two elements.
 * 交换两个元素的便捷函数。
 *
