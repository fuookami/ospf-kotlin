/**
 * 移动概念接口 / Move concept interface providing move semantics for types.
 * 为类型提供移动语义的移动概念接口。 / This interface defines a move operation that transfers ownership of resources
...
/**
 * 可移动接口 / Interface for types that support move semantics.
 * 支持移动语义的类型接口。 / Types implementing this interface can transfer their internal state
...
interface Movable<Self> {
    ...
    /**
     * 执行移动操作 / Performs a move operation, returning a new instance with the transferred state.
...
/**
 * 移动元素的便捷函数 / Convenience function to move an element.
 * 移动元素的便捷函数。
 *
