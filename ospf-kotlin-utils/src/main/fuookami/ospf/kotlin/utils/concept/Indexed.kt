/**
 * 索引概念接口 / Indexed concept interface providing index management for types.
 * 为类型提供索引管理的索引概念接口。 / This file provides interfaces and classes for automatic index assignment
...
/**
 * 索引实现基类 / Sealed base class providing index generation functionality.
 * 提供索引生成功能的密封基类。 / This class manages index counters for different types using a thread-safe
...
sealed class IndexedImpl {
    ...
    /**
     * 获取下一个索引（内联函数，使用具体化类型参数） / Gets the next index for the specified type using reified type parameter.
    ...
    */
    ...
    /**
     * 获取下一个索引 / Gets the next index for the specified class.
     * 获取指定类的下一个索引。
     *
    ...
    */
    ...
    /**
     * 重置索引计数器（内联函数，使用具体化类型参数） / Resets the index counter for the specified type using reified type parameter.
    ...
    */
    ...
    /**
     * 重置索引计数器 / Resets the index counter for the specified class.
     * 重置指定类的索引计数器。
     *
...
/**
 * 索引存储映射 / Private map storing index counters for each class type.
 * 私有映射，存储每个类类型的索引计数器。
*/
private val impls = ConcurrentHashMap<KClass<*>, AtomicInteger>()

/**
 * 可索引接口 / Interface for types that have an index.
 * 具有索引的类型接口。 / Types implementing this interface have a unique integer index
...
interface Indexed {
    ...
    /**
     * 索引值 / The unique integer index for this instance.
     * 此实例的唯一整数索引。
    */
    val index: Int

    /**
     * 无符号长整型索引 / The index as an unsigned long value.
     * 索引的无符号长整型值。
    */
    val uindex: ULong get() = index.toULong()

    /**
     * 是否已设置索引 / Returns true if the index has been set.
     * 如果索引已设置则返回 true。
    */
...
/**
 * 手动索引类 / Class for types where index assignment is manual.
 * 索引手动分配的类型类。 / Use this class when you need to control when the index is assigned.
 * 当需要控制索引分配时机时使用此类。
 *
...
open class ManualIndexed internal constructor(
    ...
    */
    ...
    /**
     * 创建未设置索引的实例 / Creates an instance without an index set.
     * 创建未设置索引的实例。
    */
    ...
    /**
     * 设置索引 / Sets the index using the instance's class type.
     * 使用实例的类类型设置索引。
    */
    fun setIndexed() {
        assert(!indexed)
    ...
    /**
     * 设置索引（指定类类型） / Sets the index using the specified class type.
     * 使用指定的类类型设置索引。
     *
    ...
    */
    ...
    /**
     * 刷新索引 / Refreshes the index with a new value using the instance's class type.
     * 使用实例的类类型刷新索引为新值。
    */
    fun refreshIndex() {
        assert(indexed)
    ...
    /**
     * 刷新索引（指定类类型） / Refreshes the index with a new value using the specified class type.
     * 使用指定的类类型刷新索引为新值。
     *
     * @param cls 要使用的类类型 / The class type to use
    */
...
/**
 * 自动索引类 / Class for types where index assignment is automatic upon creation.
 * 索引在创建时自动分配的类型类。 / Use this class when you want the index to be automatically assigned
...
open class AutoIndexed internal constructor(
    ...
    */
    ...
    /**
     * 创建带有指定类类型索引的实例 / Creates an instance with an index from the specified class type.
     * 创建带有指定类类型索引的实例。
     *
     * @param cls 要使用的类类型 / The class type to use
    */
...
/**
 * 在列表中查找或获取指定索引的元素 / Finds an element by index in the list, or gets the element at that position if not found.
 * 在列表中按索引查找元素，如果未找到则获取该位置的元素。 / This function first attempts to find an element with the matching index property.
