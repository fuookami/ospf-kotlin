/**
 * 错误类型定义 / Error type definitions for the ospf-kotlin-utils library.
...
/**
 * 错误基类 / Sealed base class for all error types in the library.
 * 库中所有错误类型的密封基类。 / This class provides a common interface for error handling with
...
sealed class Error<out C : Any> {
    companion object {
        /**
         * 创建错误实例 / Creates an Err instance with the specified code and message.
         * 使用指定的代码和消息创建 Err 实例。
         *
    ...
    /**
     * 错误代码 / The error code identifying the type of error.
     * 标识错误类型的错误代码。
    */
    abstract val code: C

    /**
     * 错误消息 / The human-readable error message.
     * 人类可读的错误消息。
    */
    abstract val message: String

    /**
     * 关联值 / An optional value associated with the error.
     * 与错误关联的可选值。
    */
    open val value: Any? = null

    /**
     * 是否有关联值 / Returns true if the error has an associated value.
     * 如果错误有关联值则返回 true。
    */
    val withValue get() = value != null

    /**
     * 返回错误字符串表示 / Returns the string representation of the error.
     * 返回错误的字符串表示。
     *
...
/**
 * 基本错误类 / Basic error class for simple error cases without associated values.
...
open class Err<out C : Any>(
    ...
    companion object {
        /**
         * 创建错误实例 / Creates an Err instance with the specified code and optional message.
...
/**
 * 惰性消息错误类 / Lazy message error class for deferring message construction until
...
open class LazyErr<out C : Any>(
    ...
    /**
     * 使用消息初始化函数创建惰性错误 / Creates a lazy error with a message initializer.
     * 使用消息初始化函数创建惰性错误。
     *
     * @param code 错误代码 / The error code
...
/**
 * 带值的扩展错误类 / Extended error class that carries an associated value of type T.
 * 携带类型 T 关联值的扩展错误类。 / This class is useful when additional context is needed for error handling,
...
open class ExErr<out C : Any, out T>(
    ...
    /**
     * 创建只带代码和值的实例 / Creates an instance with code and value, using code's string representation as message.
...
/**
 * 惰性扩展错误类 / Lazy extended error class that defers both message and associated value
...
open class LazyExErr<out C : Any, out T>(
    ...
    /**
     * 使用消息和值初始化函数创建惰性扩展错误 / Creates a lazy extended error with message and value initializers.
     * 使用消息和值初始化函数创建惰性扩展错误。
     *
     * @param code 错误代码 / The error code
     * @param message 错误消息初始化函数 / Error message initializer
    ...
    */
    ...
    /**
     * 使用惰性值创建惰性扩展错误 / Creates a lazy extended error with code's string representation as
    ...
    */
    ...
    /**
     * 使用值初始化函数创建惰性扩展错误 / Creates a lazy extended error with code's string representation as
...
/**
 * 应用程序异常 / An exception wrapper for Error instances, allowing errors to be thrown as exceptions.
...
data class ApplicationException(
    ...
    /**
     * 异常消息 / The exception message, delegated from the wrapped error.
     * 异常消息，从包装的错误委托而来。
    */
    override val message: String by error::message

    /**
     * 返回异常字符串表示 / Returns the string representation of the exception.
     * 返回异常的字符串表示。
     *
