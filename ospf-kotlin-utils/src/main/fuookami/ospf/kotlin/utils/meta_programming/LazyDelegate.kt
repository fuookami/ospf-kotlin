/**
 * 懒加载委托工具 / Lazy delegate utilities for Kotlin property delegation.
 * 提供属性委托模式下的懒加载实现，支持同步和异步懒加载。 / Provides lazy loading implementations for Kotlin property delegation,
...
/**
 * 创建懒加载委托 / Creates a lazy delegate for property delegation.
 * 创建一个懒加载委托，用于属性委托模式。
 *
...
/**
 * 创建自引用懒加载委托 / Creates a self-referencing lazy delegate for property delegation.
...
/**
 * 从属性引用创建自引用懒加载委托 / Creates a self-referencing lazy delegate from a property reference.
...
/**
 * 创建挂起懒加载 / Creates a suspend lazy initializer for coroutine-based lazy initialization.
...
/**
 * 懒加载委托类 / Lazy delegate class for property delegation with deferred initialization.
 * 懒加载委托类，支持属性委托模式下的延迟初始化。
 *
 * 值在首次访问时通过懒加载函数初始化，之后缓存该值。 / The value is initialized via the lazy function on first access and then cached.
 *
 * @param T 宿主类型 / Host type
 * @param U 值类型 / Value type
...
class LazyDelegate<T, U : Any>(
    ...
    /**
     * 缓存的值 / The cached value.
     * 缓存的懒加载值，使用 lateinit 延迟初始化。
    */
    lateinit var range: U

    /**
     * 获取属性值（委托操作符） / Gets the property value (delegation operator).
    ...
    */
    ...
    /**
     * 设置属性值（委托操作符） / Sets the property value (delegation operator).
...
/**
 * 自引用懒加载委托类 / Self-referencing lazy delegate class for property delegation with host object access.
 * 自引用懒加载委托类，允许懒加载函数访问宿主对象。
 *
 * 与 [LazyDelegate] 类似，但懒加载函数接收宿主对象作为参数， / Similar to [LazyDelegate], but the lazy function receives the host object as a parameter,
 * 允许基于宿主对象的状态进行初始化。 / allowing initialization based on the host object's state.
 *
 * @param T 宿主类型 / Host type
 * @param U 值类型 / Value type
...
/**
 * 挂起懒加载类 / Suspend lazy class for coroutine-based lazy initialization.
 * 挂起懒加载类，用于协程环境下的懒加载初始化。
 *
 * 使用原子引用和 Deferred 实现线程安全的懒加载， / Uses atomic reference and Deferred for thread-safe lazy initialization,
 * 确保在并发环境下只初始化一次。 / ensuring single initialization in concurrent environments.
 *
 * @param T 值类型 / Value type
...
class SuspendLazy<T>(
    ...
    */
    ...
    /**
     * 获取懒加载值（挂起操作符） / Gets the lazy value (suspend operator).
     * 获取懒加载值，首次调用时启动协程异步初始化。
     *
     * 使用双重检查锁定模式确保并发安全： / Uses double-checked locking pattern for concurrency safety:
