...
class FoldConstantPathTest {
    ...
    fun segmentShouldKeepFoldResultsStable() = runBlocking {
        ...
        val leftFoldSegmentOne = numbers.foldParallelly(segment = UInt64.one, initial = 0) { acc, value -> acc + value }
        val leftFoldSegmentThree = numbers.foldParallelly(segment = UInt64(3), initial = 0) { acc, value -> acc + value }
        assertEquals(10, leftFoldSegmentOne)
        assertEquals(10, leftFoldSegmentThree)

        val rightFoldSegmentOne = numbers.foldRightParallelly(segment = UInt64.one, initial = 0) { acc, value -> acc + value }
        val rightFoldSegmentThree = numbers.foldRightParallelly(segment = UInt64(3), initial = 0) { acc, value -> acc + value }
    ...
    fun zeroSegmentShouldBeRejected() {
        assertFailsWith<IllegalArgumentException> {
            runBlocking {
                listOf(1, 2, 3).foldParallelly(segment = UInt64.zero, initial = 0) { acc, value -> acc + value }
            }
        }
        assertFailsWith<IllegalArgumentException> {
            runBlocking {
                listOf(1, 2, 3).tryFoldParallelly(segment = UInt64.zero, initial = 0) { acc, value -> Ok(acc + value) }
            }
        }
        assertFailsWith<IllegalArgumentException> {
            runBlocking {
                listOf(1, 2, 3).exTryFoldParallelly(segment = UInt64.zero, initial = 0) { acc, value -> Ok(acc + value) }
            }
        }
    }
}
