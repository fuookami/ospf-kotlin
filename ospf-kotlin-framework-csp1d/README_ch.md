...
中等规模、混合需求单位和扩展规模 baseline 测试已覆盖 DFS、N-Sum、N-Same 和 FullSum，统一记录访问节点数、候选数量、接受方案数、不可行候选数、重复候选数、dominance 剪枝数、宽度上界剪枝节点数、刀数下界剪枝节点数、长度上界剪枝入口数、物料宽度索引缓存命中数、耗时和停止原因。`CuttingPlanGenerationBenchmarkSnapshot` 只保留确定性的数量字段，并通过 `toStableLine()` 输出可比较的 benchmark 快照；耗时仍留在原始统计中作为趋势观察。
