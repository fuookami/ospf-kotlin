...
/**
 * 按物料维度缓存过滤后的宽度索引，避免对同一物料重复计算。 / Caches filtered width indices by material dimension to avoid redundant computation.
...
internal class GenerationMaterialWidthIndexCache<V : RealNumber<V>>(
    ...
    /**
     * 获取指定物料的过滤后宽度索引（命中缓存时直接返回）。 / Gets the filtered width index for the specified material (returns cached entry on hit).
    ...
    */
    ...
    /**
     * 缓存中的宽度索引条目，包含原始索引和长度边界剪枝信息。 / Cached width index entry containing the original index and length-bound pruning information.
