...
CSP1D 当前已完成阶段性基础建设，可作为后续继续推进的基础。C3 已完成验收。
...
5. 已完成 application 层主问题 MILP 化改造，消除占位 solver 设计。

### 14.2 下一轮目标

~~下一轮目标保持为 C3：消除 C0-1 骨架阶段留下的占位 solver 设计，在 application 层完成真实主问题 MILP 编排。~~ ✅ C3 已完成。

下一轮目标为 C4：实现 shadow price 提取和 pricing 新列生成。
...
1. ~~C3 尚未验收完成~~：✅ 已完成。application 主问题 MILP 化已通过编译和验收测试。
2. ~~application 测试仍需同步~~：✅ 已完成。测试已改为 `runBlocking` + 显式注入 `Csp1dFakeSolver`。
...
### 14.8 C3 验证记录（2026-06-03 补充）

1. 编译通过（17 个模块，BUILD SUCCESS）：
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-application -am "-DskipTests=true" "-Dgpg.skip=true" compile`
2. 测试编译通过（17 个模块，BUILD SUCCESS）：
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-application -am "-Dgpg.skip=true" test-compile`
3. application 验收测试通过（3 tests, 0 failures, 0 errors）：
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-application -am "-Dgpg.skip=true" "-Dtest=Csp1dApplicationAcceptanceTest" "-Dsurefire.failIfNoSpecifiedTests=false" test`
4. 门禁检查：
   - `rg -n "ProduceSolver|SimpleProduceSolver" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "com\.poit|framework\.bpp3d" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "rollDemand|weightDemand|sheetDemand" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "candidatePlans" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
5. 代码空白检查：
   - `git diff --check`：无空白错误，仅有 Git LF/CRLF 提示。
6. 说明：
   - PowerShell 下 `-Dgpg.skip=true` 等参数需要加引号或使用 `--%`，否则可能被解析成错误的 Maven lifecycle phase。
   - 长时间 `-am` 编译可设置 `MAVEN_OPTS=-XX:ReservedCodeCacheSize=1g` 以减少 CodeHeap 警告；未设置时也已完成过编译和测试编译。

### 14.9 当前工作区修正摘要（2026-06-03）

1. `ProduceInput.candidatePlans` 已重命名为 `cuttingPlans`，application 调用点已同步，满足 `candidatePlans` Kotlin 门禁。
2. `Csp1dMilpSolver` 保持 application 层直接构建 `LinearMetaModel`，注册需求、物料、设备约束和批次最小化目标，并调用 `ColumnGenerationSolver.solveMILP()`。
3. `Csp1dApplicationAcceptanceTest` 已改为 `runBlocking` 调 suspend API，显式注入 `Csp1dFakeSolver`，并验证 C3 阶段列生成 trace 的 `pricedPlanCount` 为空。
4. 测试代码中旧需求口径局部变量名已改为 `rollRequirement` / `weightRequirement` / `sheetRequirement`，避免 `rollDemand|weightDemand|sheetDemand` 门禁误命中。
5. 为通过 `-am test-compile`，已做最小测试类型修正：
   - `ospf-kotlin-utils/src/test/.../ParallelConcurrencyControlTest.kt`：将默认并发量断言转为 `Long` 比较。
   - cutting-plan-generation 的 DFS / NSame / NSum 测试：将 `sumOf { it.amount }` 改为 `sumOf { it.amount.toInt() }`，避免 Kotlin unsigned overload 歧义。

### 14.10 交接给下个会话的待续项

1. ~~**优先修复 cutting-plan-generation 域测试行为失败**~~：✅ 已于 2026-06-03 修复并验证通过。
   - CostarFillerTest: 3 tests, 0 failures
   - DFSGeneratorTest: 4 tests, 0 failures
   - NSameGeneratorTest: 5 tests, 0 failures
   - NSumGeneratorTest: 3 tests, 0 failures
2. ~~**C4 shadow price 提取和 pricing 新列生成**~~：✅ 已于 2026-06-03 完成骨架并验证通过。
   - `Csp1dMilpSolver.solveLP` 已实现 LP 松弛求解和 dual solution 提取。
   - `Csp1dMilpSolver.extractShadowPrices` 已实现从 dual solution 到 `ShadowPriceMap<V>` 的转换。
   - `Csp1dColumnGeneration.solveWithTrace` 已接入 LP → shadow price → pricing generator 循环。
   - `SimplePricingGenerator` 已实现按 shadow price 触发新列生成。
   - 验收测试 3 项全部通过（含列生成 trace 验证）。
3. **增强上下文 solver 化**：yield、length assignment、wasting minimization 仍是分析层能力，后续需转为主问题约束或目标插件。
4. **Flt64/FltX 门禁全面检查**：`rg -n "Flt64|FltX" ospf-kotlin-framework-csp1d -g "*.kt"` 的 Kotlin 代码命中位置仍需逐项确认是否在允许范围内。
5. **C4 深化**：当前 `SimplePricingGenerator` 仅按 shadow price 正负判定是否生成新列，未实现 reduced cost 精确计算。后续需实现真正的 reduced cost pricing，并在真实 solver 上验证列生成收敛。
6. **切割方案生成算法补齐**：FullSum、缺陷/分段、unitBatch、单位长度、并行和时间限制仍未完成。

### 14.11 全量验证记录（2026-06-03 补充）

1. application 验收测试通过（3 tests, 0 failures）。
2. cutting-plan-generation 域测试全部通过（15 tests, 0 failures）。
3. material 域测试通过（ProductGenericTypeTest 3 tests, ProductDemandModelTest 4 tests）。
4. 增强上下文测试通过（YieldContextTest + LengthAssignmentContextTest + WastingMinimizationContextTest）。
5. 门禁检查全部通过：
   - `rg -n "ProduceSolver|SimpleProduceSolver" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "com\.poit|framework\.bpp3d" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "rollDemand|weightDemand|sheetDemand" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
...
### 14.12 C4 验证记录（2026-06-03 补充）

1. IDE 编译检查：`Csp1dColumnGeneration.kt`、`Csp1dMilpSolver.kt`、`Csp1dApplicationAcceptanceTest.kt` 均无错误。
2. application 验收测试通过（3 tests, 0 failures, exit code 0）：
   - `milpShouldSolveRollDemandWithoutPoitDependency`：通过。
   - `milpShouldCoverCostarRestWidthAndMachineCapacity`：通过。
   - `columnGenerationShouldSolveInitialPlans`：通过。
3. C4 关键修改：
   - `Csp1dMilpSolver`：新增 `solveLP` 方法，构建 LP 松弛模型并调用 `ColumnGenerationSolver.solveLP`；新增 `extractShadowPrices` 方法，从 dual solution 提取 `ShadowPriceMap<V>`；新增 `shadowPriceKeyFromName` 方法，将约束名映射为 `ShadowPriceKey`。
   - `Csp1dColumnGeneration`：列生成循环改为先调 `solveLP` 提取 shadow price，再调 pricing generator 生成新列；`UInt64` 构造统一使用 `Int` 重载。
   - `Csp1dApplicationAcceptanceTest`：fake solver 的 `solveLP` 返回空 `dualSolution`，确保 C4 路径可执行但不会因 fake 数据产生误判。
4. 门禁检查：
   - `rg -n "ProduceSolver|SimpleProduceSolver" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "com\.poit|framework\.bpp3d" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "rollDemand|weightDemand|sheetDemand" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
...
## 15. 执行进展与交接（2026-06-03）

### 15.1 本次已完成

1. 已修正 reduced cost pricing 的实际接入：
   - `Csp1dColumnGeneration` 默认 pricing generator 已改为 `ReducedCostPricingGenerator(initialGenerator)`。
   - `ReducedCostPricingGenerator` 保留 `V : RealNumber<V>` 泛型，不在领域主代码中固定使用 `Flt64` 或 `FltX`。
   - reduced cost 判定改为计算 dual benefit：`Σ(contribution * shadow_price) + material_sp + machine_sp`，`benefit > 1` 等价于 `reduced_cost < 0`。
2. 已修正 shadow price key 的口径问题：
   - `ProductDemandShadowPriceKey` 增加 `unitSymbol`，避免同一产品不同需求单位互相覆盖。
   - `SimplePricingGenerator`、`ReducedCostPricingGenerator` 和 LP shadow price 提取均使用 product id + demand unit 建 key。
3. 已修正 LP dual solution 到 `ShadowPriceMap` 的映射方式：
   - `Csp1dMilpSolver.solveLP` 在建约束时维护 `constraintName -> ShadowPriceKey` 映射。
   - 不再通过字符串拆分约束名反解析 product/material/machine id，避免 id 包含 `_` 时被截断。
4. 已修正切割方案生成器的离散需求贡献单位：
   - `NSameGenerator`、`DFSGenerator`、`NSumGenerator` 对 roll/sheet 等离散需求使用 demand 自身 unit，不再误用产品宽度的 `Meter`。
5. 已修正测试和注释：
   - `ReducedCostPricingGeneratorTest` 同步新的 `ProductDemandShadowPriceKey(productId, unitSymbol)`。
   - 测试注释改为中英双语。
   - 物料 shadow price 测试断言改为符合“shadow price 使候选不减少”的方向。

### 15.2 本次验证记录

1. 编译通过：
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-domain-cutting-plan-generation-context -am "-DskipTests=true" "-Dgpg.skip=true" compile`：BUILD SUCCESS。
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-application -am "-DskipTests=true" "-Dgpg.skip=true" compile`：BUILD SUCCESS。
2. 新增 reduced cost pricing 测试通过：
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-domain-cutting-plan-generation-context -am "-Dgpg.skip=true" "-Dtest=ReducedCostPricingGeneratorTest" "-Dsurefire.failIfNoSpecifiedTests=false" test`：BUILD SUCCESS，5 tests, 0 failures。
3. application 验收测试通过：
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-application -am "-Dgpg.skip=true" "-Dtest=Csp1dApplicationAcceptanceTest" "-Dsurefire.failIfNoSpecifiedTests=false" test`：BUILD SUCCESS，3 tests, 0 failures。
4. 空白检查：
   - `git diff --check -- ospf-kotlin-framework-csp1d`：无空白错误，仅 Git LF/CRLF 提示。
5. 门禁检查：
   - `rg -n "ProduceSolver|SimpleProduceSolver" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "com\.poit|framework\.bpp3d" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "rollDemand|weightDemand|sheetDemand" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `rg -n "candidatePlans" ospf-kotlin-framework-csp1d -g "*.kt"`：无命中。
   - `ProductDemandShadowPriceKey(` 调用点已全部同步 `unitSymbol`。

说明：本次未重新声明“全量测试通过”；当前仅记录上述实际执行过的编译、目标测试和门禁结果。

### 15.3 交接给下个会话的待续项

1. **C4 列生成收敛验证**：`ReducedCostPricingGenerator` 已接入默认列生成流程，但当前 application 验收仍基于 fake solver，无法验证真实 solver 上的 LP 迭代收敛质量。后续需在真实 solver 上验证列生成是否收敛到 LP 最优。
2. **C5 增强上下文 solver 化**：yield、length assignment、wasting minimization 仍是分析层能力，后续需转为主问题约束或目标插件（如最小化欠产惩罚、超产上限约束、动态卷长变量、余宽浪费目标）。
3. **切割方案生成算法补齐**：FullSum、缺陷/分段、unitBatch、单位长度、并行和时间限制仍未完成。
4. **pricing 子问题性能优化**：当前默认 `ReducedCostPricingGenerator` 复用 `SimpleInitialCuttingPlanGenerator` 枚举候选；后续可按场景注入 `DFSGenerator`、`NSumGenerator` 或更高效的专用 pricing 子问题。
5. **Flt64/FltX 门禁持续检查**：本次已避免在 reduced cost 主逻辑中固定 `Flt64`/`FltX`，后续新增领域主代码仍需保持泛型数值口径。

### 15.4 下个会话启动建议

1. 优先使用真实 `ColumnGenerationSolver` 构造一个小规模 CSP1D 样例，验证 LP → dual price → reduced cost pricing → 新列加入的完整迭代链路。
2. 验证时重点观察：
   - 每轮 `pricedPlanCount` 是否逐步降为 0。
   - `ProductDemandShadowPriceKey(productId, unitSymbol)` 是否覆盖 roll/sheet/weight 等不同单位需求。
   - 含 `_` 的 product/material/machine id 是否能正确提取 shadow price。
3. 建议先跑窄范围命令，避免全量 reactor 成本过高：
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-domain-cutting-plan-generation-context -am "-Dgpg.skip=true" "-Dtest=ReducedCostPricingGeneratorTest" "-Dsurefire.failIfNoSpecifiedTests=false" test`
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-application -am "-Dgpg.skip=true" "-Dtest=Csp1dApplicationAcceptanceTest" "-Dsurefire.failIfNoSpecifiedTests=false" test`
