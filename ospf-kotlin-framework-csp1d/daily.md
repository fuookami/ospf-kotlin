...
59. 已完成生成器物料等价宽度入口复用第一阶段：DFS、NSum、FullSum 可按等价幅宽轮廓复用过滤后的产品-宽度入口索引，新增 `materialWidthIndexCacheHits` 统计、benchmark 快照字段、KPI/render key 和等价物料验收覆盖。
60. 已完成生成器刀数下界可达性剪枝第一阶段：DFS、NSum、FullSum 可提前剪掉无法达到 `minKnifeCount` 的组合分支，新增 `knifeBoundPrunedNodes` 统计、benchmark 快照字段、KPI/render key 和最小刀数不可达验收覆盖。
61. 已完成当前验证基线刷新：CSP1D 聚焦测试集合、Gurobi profile `test-compile`、CSP1D 门禁搜索和 `git diff --check` 均已通过。
62. 已完成生成器扩展规模 benchmark 快照第一阶段：新增更大物料池、更多宽度分布和更高候选上限的四算法稳定快照，覆盖宽度上界、刀数下界、长度上界、物料等价入口缓存和基础可行性统计口径。
63. 已完成当前验证基线刷新：CSP1D 聚焦测试集合、Gurobi profile `test-compile`、CSP1D 门禁搜索和 `git diff --check` 均已通过。
64. 已完成生成器跨物料切片模板复用第一阶段：DFS、NSum、FullSum 在顺序生成和内置约束路径下可按等价幅宽轮廓复用切片组合模板，并按当前物料重新构造方案、需求贡献和动态长度贡献。
65. 已完成当前验证基线刷新：CSP1D 局部测试、Gurobi profile `test-compile`、CSP1D 门禁搜索和 `git diff --check` 均已通过。
...
1. DFS/NSum/NSame/FullSum 已可用，基础贡献构造、可行性入口、统一候选上限、统计报告、数量缓存、宽度索引剪枝、长度上界剪枝、刀数下界可达性剪枝、物料等价宽度入口复用、物料等价切片模板复用、按物料并行、同贡献 dominance、四算法中等规模基线、混合单位和扩展规模稳定快照已完成第一阶段。
2. 单位长度、更细粒度动态长度边界、更大规模 benchmark、更多约束/并行边界下的跨物料模板复用和更强 dominance 剪枝仍待深化。
3. 缺陷、分段、onSide/inMiddle、`unitBatch` 等 POIT 语义暂不属于当前实体边界；只有当 material model 增加通用实体后再纳入。

#### 计划

1. 继续扩展 width combination 的上界估计、组合 dominance、更多约束/并行边界下的跨物料模板复用和更细动态长度边界剪枝。
2. 将稳定 benchmark 快照继续扩展到更多约束组合、设备可行性场景和更高候选上限场景，耗时类指标只作为趋势观察。
...
3. 中等规模、混合单位和更大规模扩展样例的候选数量、重复过滤、dominance 剪枝、宽度上界剪枝、刀数下界剪枝、长度上界剪枝、物料等价宽度入口缓存命中、物料等价切片模板缓存命中和基础可行性拒绝有稳定快照记录。
...
2. 在已完成数量缓存、宽度索引剪枝、长度上界剪枝、刀数下界可达性剪枝、物料等价宽度入口复用、物料等价切片模板复用、并行开关、同贡献 dominance、四算法中等规模 baseline 和多场景稳定快照的基础上，推进组合 dominance、更多约束/并行边界下的跨物料模板复用和更细动态长度剪枝。
3. 将生成统计继续沉淀到 application KPI/render 边界，并把 benchmark 快照继续扩展到更多约束组合和设备可行性基线。
...
1. 第一段：深化生成算法剪枝，覆盖组合 dominance、更多约束/并行边界下的跨物料模板复用、动态长度边界剪枝和结果 canonical 稳定性。
2. 第二段：将四算法 benchmark 快照继续扩展到更多约束组合和设备可行性基线，明确耗时类统计与数量类统计的验收口径。
...
3. benchmark 快照能比较 DFS、NSum、NSame、FullSum 在中等规模、混合单位和更大规模场景下的候选数量、重复过滤、dominance 剪枝、宽度上界剪枝、刀数下界剪枝、长度上界剪枝、物料等价宽度入口缓存命中、物料等价切片模板缓存命中和基础可行性拒绝，耗时只作为趋势观察。
