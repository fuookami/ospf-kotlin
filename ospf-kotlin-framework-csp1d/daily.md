...
1. **优先修复 cutting-plan-generation 域测试行为失败**：补跑 CSP1D 指定域测试时，material 测试已通过（14 tests, 0 failures），但 cutting-plan-generation 失败 5 项：
   - `CostarFillerTest.singleCostarFillsRemainingWidth`
   - `CostarFillerTest.multipleCostarsCombination`
   - `DFSGeneratorTest.knifeCountPruning`
   - `NSameGeneratorTest.knifeCountConstraint`
   - `NSumGeneratorTest.depthLimitIsRespected`
2. **失败命令记录**：
   - `mvn -ntp -pl ospf-kotlin-framework-csp1d/csp1d-domain-material-context,ospf-kotlin-framework-csp1d/csp1d-domain-cutting-plan-generation-context,ospf-kotlin-framework-csp1d/csp1d-domain-yield-context,ospf-kotlin-framework-csp1d/csp1d-domain-length-assignment-context,ospf-kotlin-framework-csp1d/csp1d-domain-wasting-minimization-context -am "-Dgpg.skip=true" "-Dtest=ProductDemandModelTest,ProductGenericTypeTest,QuantityArithmeticTest,CostarFillerTest,DFSGeneratorTest,NSameGeneratorTest,NSumGeneratorTest,YieldContextTest,LengthAssignmentContextTest,WastingMinimizationContextTest" "-Dsurefire.failIfNoSpecifiedTests=false" test`
3. **初步排查方向**：
   - `CostarFiller.fill(...)` 当前依赖 `plan.restWidth`；测试中的 `material.widthRange` 上界为 2.0，产品宽 1.0，配规宽 0.5，预期应产生含配规 slice 的方案，但当前没有命中，需检查 `restWidth` 计算、`partialOrd` 判断或 `fillDFS` 分支。
   - DFS / NSame / NSum 三个失败用例均先断言 `plans.isNotEmpty()`，说明当前生成器在刀数/深度约束输入下返回了空方案；需检查 `material.widthRange.width.contains(productWidth)`、`QuantityRange.contains`、`computeMaxByWidth` 和需求贡献单位语义。
4. **不要把上述失败和 C3 application 验收混淆**：C3 application 主问题 MILP 化已通过编译、测试编译和验收测试；cutting-plan-generation 行为失败属于 C2/CSP1D 域算法回归或测试语义不一致，应先修复再进入 C4。
5. **C4 shadow price 提取和 pricing 新列生成**：当前 `Csp1dColumnGeneration.solveWithTrace` 仅复用主问题 MILP，`pricedPlanCount` 为空列表。待 cutting-plan-generation 域测试恢复后，再从 LP 松弛对偶解提取 shadow price，调用 pricing generator 生成 reduced cost 为负的新切割方案列。
6. **增强上下文 solver 化**：yield、length assignment、wasting minimization 仍是分析层能力，后续需转为主问题约束或目标插件。
7. **Flt64/FltX 门禁全面检查**：`rg -n "Flt64|FltX" ospf-kotlin-framework-csp1d -g "*.kt"` 的 Kotlin 代码命中位置仍需逐项确认是否在允许范围内。
