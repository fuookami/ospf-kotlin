...
C4 真实 solver 列生成端到端验证已完成。C5 yield/waste/length 真实 solver 回填值验证已完成。yield/waste 建模与列生成的交互分析已完成（确认 LP 轮次不需要 yield slack 和 waste 目标项）。

下一会话建议优先顺序：

1. 补 yield 建模：sheet、weight 需求口径下的端到端建模测试（当前仅覆盖 roll）。
2. 补 wasting 建模：不同物料宽度范围、多物料和多需求场景下的端到端建模测试。
3. 补 length assignment 建模：动态长度产品与固定长度产品的建模差异和 `Quantity<V>` 单位一致性端到端测试。
4. 继续推进切割方案生成算法补齐（3.5 节）。
5. 继续执行门禁搜索，避免引入 `com.poit`、`framework.bpp3d`、`ProduceSolver`、`SimpleProduceSolver`、`candidatePlans` 或不合规的固定数值类型。

#### 当前验证记录

本轮实际执行并通过：

1. `mvn -pl ospf-kotlin-framework-csp1d/csp1d-application -am -Pgurobi-cg-test -o test`：35 tests, 0 failures（含 20 fake solver 验收测试 + 15 Gurobi 真实 solver 测试，含新增 8 个 yield/waste/length 端到端验证测试）。
