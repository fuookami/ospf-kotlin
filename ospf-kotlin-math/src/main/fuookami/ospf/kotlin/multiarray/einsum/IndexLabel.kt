/**
 * 索引标签和索引列表定义 / Index labels and index lists definitions
 *
 * 提供爱因斯坦表示法的索引标记系统。 / Provides index label system for Einstein notation.
 *
 * 圌Rust 中使用类型级别的编程实现编译期索引跟踪，
 * 圌Kotlin 中使用枚举和运行时列表实现类似功能。 / In Rust, type-level programming is used for compile-time index tracking.
 ...
 * 索引标签 / Index label
 *
 * 用于标识张量的索引维度。 / Used to identify tensor dimension indices.
 *
 * 内置索引标签， / Built-in index labels:
...
enum class IndexLabel(
    /**
     * 索引名称（用于调试和显示， / Index name (for debugging and display)
    */
    val labelName: String,

    /**
     * 索引的唯一标识笌 / Unique identifier for the index
     *
     * 用于在运行时比较索引是否相同。 / Used to compare indices at runtime.
    */
    val id: Int
) {
    ...
    companion object {
        /**
         * 根据名称查找索引标签 / Find index label by name
         *
        ...
        */
        ...
        /**
         * 根据 ID 查找索引标签 / Find index label by ID
         *
         * @param id 索引 ID，-5，
         * @return 对应的索引标签，如果不存在则返回 null
        */
 ...
 * 索引列表 / Index list
 *
 * 表示一组索引的有序集合。 / Represents an ordered collection of indices.
 *
 * 示例 / Example:
 *
 * ```kotlin
...
data class IndexList(
    ...
    */
    ...
    /**
     * 将索引列表转换为索引 ID 列表 / Convert index list to list of index IDs
    */
    val ids: List<Int> = indices.map { it.id }

    /**
     * 将索引列表转换为名称字符串 / Convert index list to name string
    */
    ...
    */
    ...
    /**
     * 检查是否包含指定索引 / Check if contains specified index
     *
    ...
    */
    ...
    /**
     * 检查是否包含指定索引ID / Check if contains specified index ID
     *
     * @param id 要检查的索引 ID
     * @return 如果包含则返回true
    */
    ...
    /**
     * 获取指定位置索引的维度位罌 / Get dimension position for index at specified position
     *
     * @param label 索引标签
    ...
    */
    ...
    /**
     * 获取指定索引 ID 的维度位罌 / Get dimension position for specified index ID
     *
     * @param id 索引 ID
     * @return 该索引在列表中的位置，如果不存在则返回null
    */
    ...
    companion object {
        /**
         * 空索引列表 / Empty index list
        */
        val Empty = IndexList(emptyList())

        /**
         * 从索引标签创建索引列表 / Create index list from index labels
         *
         * @param labels 索引标签 / the index labels
         * @return 创建的索引列表 / the created index list
        */
        fun of(vararg labels: IndexLabel): IndexList = IndexList(labels.toList())

        /**
         * 从索引ID 创建索引列表 / Create index list from index IDs
         *
         * @param ids 索引 ID 列表 / the index IDs
         * @return 创建的索引列表 / the created index list
        */
        ...
        /**
         * 从名称字符串解析索引列表 / Parse index list from name string
         *
...
/**
 * 查找两个索引列表的公共索引（求和索引， / Find common indices (summation indices) between two index lists
 *
 * 爱因斯坦表示法中，公共索引表示需要求和的维度。 / In Einstein notation, common indices represent dimensions to be summed over.
 *
 * 示例 / Example:
 *
 * ```kotlin
...
/**
 * 从索引列表中移除指定索引 / Remove specified indices from an index list
 *
 * 示例 / Example:
 *
 * ```kotlin
...
/**
 * 合并两个索引列表，保持唯一怌 / Merge two index lists, maintaining uniqueness
 *
 * @param lhs 左操作数的索引列表
 * @param rhs 右操作数的索引列表
 * @return 合并后的唯一索引列表
