 ...
 *
 * @property interval 对应的 Interval 枚举值
 */
 ...
 *
 * 类型推导规则：
 * - plus/minus/times/div: 先计算 ValueRange 运算结果，再依据结果上下界区间推导 Closed/Open typed kind
 ...
 * @param LB 下边界区间类型
 * @param UB 上边界区间类型
 * @property valueRange 内部的值范围
 * @property lowerKind 下边界区间类型标记
 * @property upperKind 上边界区间类型标记
 */
class TypedValueRange<T, LB : IntervalKind, UB : IntervalKind> private constructor(
     ...
     *
     * @param interval Interval 枚举值
     * @return 对应的 IntervalKind
     */
    ...
    /**
     * 将值范围转换为指定类型标记的类型化值范围
     * Converts value range to typed value range with specified kind markers
     *
     * @param NLB 新的下边界区间类型
     * @param NUB 新的上边界区间类型
     * @param range 要转换的值范围
     * @param lowerKind 下边界区间类型标记
     * @param upperKind 上边界区间类型标记
     * @return 指定类型标记的类型化值范围，或 null
     */
     ...
     *
     * @param rhs 另一个同类型的类型化值范围
     * @return 相同类型标记的类型化值范围，或 null
     */
