 ...
 * 求和扩展 / Sum Extension
 *
 * 为支持加法运算和具有零常量的类型提供求和扩展函数。 / Provides sum extension functions for types that support addition and have a zero constant.
 *
 * 此扩展适用于任何满足以下条件的类型 T，
 * 1. 实现 Plus<T, T>（支挌+ 运算符）
 * 2. 具有包含零常量的伴生对象（通过 HasZero<T>， / This extension works for any type T that:
...
/**
 * 对可迭代对象中所有元素求和。
 * 如果可迭代对象为空，返回零。 / Sums all elements in an iterable.
...
/**
 * 对数组中所有元素求和。
 * 如果数组为空，返回 Failed。 / Sums all elements in an array.
...
/**
 * 对数组中所有元素求和，空数组返回 null。 / Sums all elements in an array, returning null for empty arrays.
 *
...
/**
 * 对数组中所有元素求和。 / Sums all elements in an array.
 *
...
/**
 * 使用显式零值对可迭代对象求和，适用于空集合。 / Sums with explicit zero for empty collections.
 *
 * @param zero 空集合时的零倌
 * @return 所有元素的和
 * @return The sum of all elements
...
/**
 * 富Int32 值集合求和。 / Sums Int32 values.
 *
 * @return 所朌Int32 值的和
...
/**
 * 富Int32 值集合求和。 / Sums Int32 values in a collection.
 *
 * @return 所朌Int32 值的和
 * @return The sum of all Int32 values
*/
