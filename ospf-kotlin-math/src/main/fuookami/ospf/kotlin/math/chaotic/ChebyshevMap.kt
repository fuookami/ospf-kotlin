/**
 * 切比雪夫映射 / Chebyshev Map
 *
 * 切比雪夫映射是基于切比雪夫多项式的一维混沌映射。
 * 该映射利用切比雪夫多项式的性质产生混沌序列，具有良好的遍历性和随机性。
 * 常用于混沌加密、伪随机数生成和混沌优化算法。 / The Chebyshev map is a one-dimensional chaotic map based on Chebyshev polynomials.
...
data class ChebyshevMap<V : FloatingNumber<V>>(
     ...
     * 将 cos 运算结果转换为类型 V。 / Safety invariant: V implements FloatingNumber<V>, and cos keeps the same runtime numeric type as input.
    ...
    */
     ...
     * 将 acos 运算结果转换为可空类型 V。 / Safety invariant: acos returns the same numeric family as input in-domain; out-of-domain remains null.
    ...
    */
    ...
    companion object {
         ...
         *
         * @param a 切比雪夫多项式阶数参数 / the Chebyshev polynomial order parameter
         * @return 新的切比雪夫映射实例 / a new ChebyshevMap instance
        */
        operator fun invoke(
 ...
 *
 * @property chebyshevMap 底层切比雪夫映射实例 / the underlying Chebyshev map instance
...
data class ChebyshevMapGenerator(
    ...
    companion object {
         ...
         *
         * @param a 切比雪夫多项式阶数参数 / the Chebyshev polynomial order parameter
         * @param x 初始状态值 / the initial state value
         * @return 新的切比雪夫映射生成器实例 / a new ChebyshevMapGenerator instance
        */
        operator fun invoke(
            a: Flt64 = Random.nextFlt64(Flt64.two, Flt64.ten),
