 ...
 * 边界情况：空集合返回 one，负数取绝对值后计算。 / Provides functionality for computing the greatest common divisor of integers,
 ...
 *
 * 通过反复相减将较大值减小，直到其中一个为零。 / Repeatedly subtracts the smaller value until one becomes zero.
 *
 ...
 *
 * 使用欧几里得算法（辗转相除法），比减法版本效率更高。 / Uses the Euclidean algorithm (division-based method), more efficient than the subtraction version.
 *
 ...
 *
 * 通过迭代对每对相邻元素调用取模 GCD 实现累积计算。 / Computes by iteratively calling modulo GCD on each adjacent pair.
 *
...
/**
 * 扩展欧几里得算法结果 / Extended Euclidean algorithm result
 *
