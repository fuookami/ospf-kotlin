 ...
 *
 * 提供整数质因数分解及相关功能�?
 * 数学定义：将正整�?n 分解为质数的乘积，n = p1^e1 * p2^e2 * ... * pk^ek�?
 * 其中 pi 为质数，ei 为对应质数的指数�?
 * 边界情况：n <= 1 返回空列表，表示无质因数分解�?
 * 因式分解使用筛法预先生成质数表，只遍历到 sqrt(n) 的质数进行试除�?
 * 反因式分解（defactorize）将质因数分解结果还原为原整数�?
 * 因数计算（divisors）返回所有能整除 n 的因数，包括 1 �?n 本身�?
 * 欧拉函数（eulerTotient）计算小�?n 的正整数中与 n 互质的个数，
 * 公式：phi(n) = n * (1 - 1/p1) * (1 - 1/p2) * ...，其�?pi �?n 的质因数�?
 *
