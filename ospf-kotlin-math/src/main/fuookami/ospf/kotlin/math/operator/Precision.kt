/**
 * 精度比较工具 / Precision Comparison Utility
 *
 * 提供基于精度的数值比较功能，用于处理浮点数等实数的近似相等和比较。
 * 由于浮点数存在精度问题，直接使用 == 比较可能导致错误结果，
 * 本模块提供了考虑精度的比较方法。 / Provides precision-based numeric comparison functionality for handling approximate
...
/**
 * 精度比较籌 / Precision Comparison Class
 *
 * 提供基于指定精度的数值比较扩展函数。
 * 所有比较操作都考虑精度容差，适用于浮点数等实数类型的比较。 / Provides extension functions for precision-based numeric comparison.
...
class Precision<T>(
    ...
    /**
     * 比较精度（取绝对值确保非负） / Comparison precision (absolute value ensures non-negative)
    */
    private val precision = precision.abs()

    /**
     * 判断两个数是否在精度范围内相筌 / Determines if two numbers are equal within precision tolerance
     *
    ...
    */
    ...
    /**
     * 比较两个数的大小关系 / Compares the order relationship between two numbers
     *
     * @param other 要比较的另一个数
    ...
    */
    ...
    /**
     * 判断两个数是否在精度范围内不相等 / Determines if two numbers are unequal within precision tolerance
     *
     * @param other 要比较的另一个数
    ...
    */
    ...
    /**
     * 判断当前数是否小于另一个数（考虑精度， / Determines if the current number is less than another (with precision)
     *
     * @param other 要比较的另一个数
    ...
    */
    ...
    /**
     * 判断当前数是否小于等于另一个数（考虑精度， / Determines if the current number is less than or equal to another (with precision)
     *
     * @param other 要比较的另一个数
    ...
    */
    ...
    /**
     * 判断当前数是否大于另一个数（考虑精度， / Determines if the current number is greater than another (with precision)
     *
     * @param other 要比较的另一个数
    ...
    */
    ...
    /**
     * 判断当前数是否大于等于另一个数（考虑精度， / Determines if the current number is greater than or equal to another (with precision)
     *
     * @param other 要比较的另一个数
...
/**
 * 创建精度比较工具实例（使用实数常量） / Creates a precision comparison utility instance (using real number constants)
 *
 * @param T 数值类垌
...
/**
 * 创建精度比较工具实例（使用具象化类型， / Creates a precision comparison utility instance (using reified type)
 *
 * @param T 数值类垌
