 ...
 * 容差是精度控制的核心概念，允许在一定误差范围内判断两个值是否相等。 / Defines tolerance-related interfaces for supporting tolerance-based equality
 ...
 *
 * 表示一个具有容差值的类型。
 * 实现此接口的类型可以提供其容差值用于比较操作。 / Represents a type with a tolerance value.
...
interface Tolerance<T> {
    ...
    /**
     * 容差值，用于判断两个值是否在误差范围内相筌 / Tolerance value used to determine if two values are equal within error range
    */
    val tolerance: T
}

/**
 * 带容差的相等判断接口 / Toleranced Equality Interface
 *
 * 定义带容差参数的相等判断函数式接口。
 * 用于判断两个值是否在指定容差范围内相等。 / Defines a functional interface for tolerance-based equality comparison.
...
fun interface TolerancedEq<T> {
    ...
    /**
     * 判断两个值是否在指定容差范围内相筌 / Determines if two values are equal within the specified tolerance
     *
     * @param lhs 左操作数
     * @param rhs 右操作数
     * @param tolerance 容差倌
...
/**
 * 带容差的顺序比较接口 / Toleranced Ordering Interface
 *
 * 定义带容差参数的顺序比较函数式接口。
 * 用于比较两个值的大小关系，考虑容差范围。 / Defines a functional interface for tolerance-based ordering comparison.
...
fun interface TolerancedOrd<T> {
    ...
    /**
     * 比较两个值的大小关系，考虑指定容差 / Compares the ordering relationship of two values, considering specified tolerance
     *
     * @param lhs 左操作数
     * @param rhs 右操作数
     * @param tolerance 容差倌
...
/**
 * 绝对容差籌 / Absolute Tolerance Class
 *
 * 表示一个绝对容差值。
 * 绝对容差是一个固定数值，直接用于比较两数之差。 / Represents an absolute tolerance value.
