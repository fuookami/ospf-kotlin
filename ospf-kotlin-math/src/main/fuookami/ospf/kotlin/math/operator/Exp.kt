/**
 * 指数运算笌 / Exponential Operator
 *
 * 定义指数运算接口，计算自然常敌e 的指定次幂。
 * 适用于所有实数类型，是自然对数的逆运算。 / Defines the exponential operation interface for computing e raised to a given power.
 * Applicable to all real number types, and is the inverse operation of natural logarithm.
 *
 * 数学定义 / Mathematical definition:
 * exp(x) = eˣ
 *
 * 其中 e 是自然常数，约等二2.71828。 / Where e is Euler's number, approximately 2.71828.
 *
 ...
 * 指数函数是自然对数的逆函数。 / Defines the exponential operation, computing e raised to the power of the current value.
...
interface Exp<out Ret> {
    ...
    /**
     * 计算指数倌eˣ / Calculates the exponential value eˣ
     *
     * @return e 的当前值次幌
     *
    */
 ...
 * 精度参数可用于控制高精度计算的行为。 / Extends the Exp interface, supporting exponential operations with specified precision parameters.
...
interface ExpP<Ret> : Exp<Ret> {
    ...
    /**
     * 计算指数倌eˣ，带精度参数 / Calculates the exponential value eˣ with precision parameters
     *
     * @param digits 有效数字位数
     * @param precision 精度倌
     * @return e 的当前值次幌
