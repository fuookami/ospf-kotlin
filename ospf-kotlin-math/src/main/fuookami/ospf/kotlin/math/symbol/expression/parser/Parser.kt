/**
 * 布尔表达式解析器 / Boolean Expression Parser
 *
...
class Parser(private val tokens: List<Token>, private val input: String? = null) {
    ...
    /**
     * 解析布尔表达式 / Parse boolean expression.
     *
    ...
    */
     ...
     * @param position 输入中发生错误的位置 / the position in the input where the error occurred
     * @return 包含解析问题的失败结果 / a failed result containing the parse issue
    */
    ...
    /**
     * 解析 or 表达式（最低优先级） / Parse or expression (lowest precedence)
     *
     * @return 解析后的 or 表达式或失败原因 / the parsed or expression or failure
    */
    ...
    /**
     * 解析 and 表达式 / Parse and expression
     *
     * @return 解析后的 and 表达式或失败原因 / the parsed and expression or failure
    */
    ...
    /**
     * 解析 not 表达式 / Parse not expression
     *
     * @return 解析后的 not 表达式或失败原因 / the parsed not expression or failure
    */
     ...
     *
     * @return 解析后的基本表达式或失败原因 / the parsed primary expression or failure
    */
    ...
    /**
     * 解析路径表达式（标识符开头的表达式） / Parse path expression (expression starting with identifier)
     *
     * @return 解析后的路径表达式或失败原因 / the parsed path expression or failure
    */
    ...
    /**
     * 解析空值检查 / Parse null check
     *
     * @param path 待检查的属性路径 / the property path to check
     * @return 空值检查表达式 / the null check expression
    */
    ...
    /**
     * 解析集合成员判断 / Parse set membership (in) expression
     *
     * @param path 左操作数的属性路径 / the property path for the left operand
     * @param negated 是否为取反的成员判断（not in） / whether the membership check is negated (not in)
     * @return 集合成员判断表达式 / the in expression
    */
    ...
    /**
     * 解析模式匹配表达式 / Parse pattern match expression
     *
     * @param path 左操作数的属性路径 / the property path for the left operand
     * @param mode 模式匹配模式 / the pattern match mode
     * @param negated 是否为取反的模式匹配 / whether the pattern match is negated
     * @return 模式匹配表达式 / the pattern match expression
    */
    ...
    /**
     * 解析比较表达式 / Parse comparison expression
     *
     * @param leftPath 左操作数的属性路径 / the property path for the left operand
     * @return 比较表达式或失败原因 / the comparison expression or failure
    */
    ...
    /**
     * 解析标量值（常量或路径引用） / Parse scalar value (constant or path reference)
     *
     * @return 解析后的标量表达式或失败原因 / the parsed scalar expression or failure
    */
    ...
    /**
     * 解析属性路径 / Parse property path
     *
     * @return 解析后的属性路径或失败原因 / the parsed property path or failure
    */
    ...
    */
     ...
     *
     * @return 前进之前的词法单元 / the token before advancing
    */
     ...
     * @param message 词法单元不匹配时的错误信息 / the error message if the token does not match
     * @return 匹配的词法单元或失败原因 / the matched token or failure
    */
    ...
    /**
     * 合并 or 表达式（扁平化） / Merge or expressions (flatten)
     *
     * @param left 左侧布尔表达式 / the left boolean expression
     * @param right 右侧布尔表达式 / the right boolean expression
     * @return 合并后的 or 表达式 / the merged or expression
    */
    ...
    /**
     * 合并 and 表达式（扁平化） / Merge and expressions (flatten)
     *
     * @param left 左侧布尔表达式 / the left boolean expression
     * @param right 右侧布尔表达式 / the right boolean expression
     * @return 合并后的 and 表达式 / the merged and expression
    */
...
/**
 * 解析布尔表达式字符串 / Parse boolean expression string
 *
...
/**
 * 尝试解析布尔表达式字符串 / Try to parse boolean expression string
 *
 * @param input 布尔表达式字符串 / Boolean expression string
