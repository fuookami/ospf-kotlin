/**
 * 表达式类型分类 / Expression Category
 *
 * 定义符号表达式按次数的分类体系，包括线性、二次、标准和非线性。
 * 用于在编译时和运行时区分表达式的类型，支持优化和类型检查。 / Defines the classification hierarchy for symbolic expressions by degree,
...
/**
 * 表达式类型分类基类 / Expression Category Base Class
 *
...
sealed class Category {
    ...
    /**
     * 返回两个分类中较高的一个。 / Returns the higher of two categories.
     *
...
/**
 * 线性表达式类型 / Linear Expression Category
 *
 * 表示线性表达式（次数为1的多项式）。 / Represents linear expressions (polynomials of degree 1).
*/
...
/**
 * 二次表达式类型 / Quadratic Expression Category
 *
 * 表示二次表达式（次数为2的多项式）。 / Represents quadratic expressions (polynomials of degree 2).
*/
...
/**
 * 标准表达式类型 / Standard Expression Category
 *
 * 表示标准形式的多项式表达式。 / Represents polynomial expressions in standard form.
*/
...
/**
 * 非线性表达式类型 / Nonlinear Expression Category
 *
 * 表示非线性表达式（次数大于2或包含非线性运算）。 / Represents nonlinear expressions (degree greater than 2 or containing nonlinear operations).
*/
...
/**
 * 比较两个分类的大小顺序。 / Compares the order of two categories.
 *
 * @param rhs 另一个分类 / Another category
...
/**
 * 返回两个分类中较高的一个。 / Returns the maximum of two categories.
 *
...
/**
 * 返回集合中最高的分类。 / Returns the maximum category in a collection.
 *
...
/**
 * 返回集合中最高的分类，如果集合为空则返回null。 / Returns the maximum category in a collection, or null if empty.
 *
