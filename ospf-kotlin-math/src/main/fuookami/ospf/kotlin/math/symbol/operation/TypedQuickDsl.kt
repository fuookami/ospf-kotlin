...
class TypedQuickDsl<V>(private val bridge: Flt64Bridge<V>) where V : NumberField<V>, V : RealNumber<V> {
    ...
    fun LinearPolynomial(): LinearPolynomial<V> =
        LinearPolynomial(emptyList(), bridge.zero)

    fun LinearPolynomial(constant: Flt64): LinearPolynomial<V> =
        LinearPolynomial(emptyList(), bridge.intoValue(constant))
    ...
    fun LinearPolynomial(monomial: LinearMonomial<V>): LinearPolynomial<V> =
        LinearPolynomial(listOf(monomial), bridge.zero)

    fun LinearPolynomial(symbol: Symbol): LinearPolynomial<V> =
        LinearPolynomial(listOf(LinearMonomial(bridge.one, symbol)), bridge.zero)

    fun MutableLinearPolynomial(): MutableLinearPolynomial<V> =
        MutableLinearPolynomial(emptyList(), bridge.zero)

    // ========== QuadraticPolynomial constructors ==========

    fun QuadraticPolynomial(): QuadraticPolynomial<V> =
        QuadraticPolynomial(emptyList(), bridge.zero)

    fun QuadraticPolynomial(constant: Flt64): QuadraticPolynomial<V> =
        QuadraticPolynomial(emptyList(), bridge.intoValue(constant))
    ...
    fun QuadraticPolynomial(monomial: QuadraticMonomial<V>): QuadraticPolynomial<V> =
        QuadraticPolynomial(listOf(monomial), bridge.zero)

    fun QuadraticPolynomial(symbol: Symbol): QuadraticPolynomial<V> =
        QuadraticPolynomial(listOf(QuadraticMonomial.linear(bridge.one, symbol)), bridge.zero)

    fun MutableQuadraticPolynomial(): MutableQuadraticPolynomial<V> =
        MutableQuadraticPolynomial(emptyList(), bridge.zero)

    // ========== Linear aggregation ==========

    fun <E> sumVars(items: Iterable<E>, selector: (E) -> Symbol?): LinearPolynomial<V> {
        val monomials = items.mapNotNull(selector).map { LinearMonomial(bridge.one, it) }
        return LinearPolynomial(monomials, bridge.zero)
    }

    fun sum(symbols: Iterable<Symbol>): LinearPolynomial<V> =
        LinearPolynomial(symbols.map { LinearMonomial(bridge.one, it) }, bridge.zero)

    // ========== Quadratic aggregation ==========

    fun <E> qsumVars(items: Iterable<E>, selector: (E) -> Symbol?): QuadraticPolynomial<V> {
        val monomials = items.mapNotNull(selector).map { QuadraticMonomial.linear(bridge.one, it) }
        return QuadraticPolynomial(monomials, bridge.zero)
    }

    fun qsum(symbols: Iterable<Symbol>): QuadraticPolynomial<V> =
        QuadraticPolynomial(symbols.map { QuadraticMonomial.linear(bridge.one, it) }, bridge.zero)
}
