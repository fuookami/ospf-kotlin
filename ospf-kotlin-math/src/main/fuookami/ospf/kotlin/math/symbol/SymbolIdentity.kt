/**
 * 符号标识 / Symbol Identity
 *
 * 提供符号的唯一标识机制，用于在符号计算过程中区分不同的符号实例。
 * 即使两个符号具有相同的名称，它们也可以通过唯一标识进行区分。 / Provides unique identification mechanism for symbols,
...
/**
 * 符号标识值类 / Symbol Identifier Value Class
 *
 * 封装符号的唯一标识字符串，提供类型安全的标识符。 / Wraps the unique identifier string of a symbol, providing type-safe identification.
 *
...
value class SymbolId(val value: String) {
    ...
    /**
     * 返回符号标识的字符串表示。 / Returns the string representation of the symbol identifier.
     *
...
/**
 * 稳定符号接口 / Stable Symbol Interface
 *
 * 提供稳定标识的符号接口，实现此接口的符号具有在生命周期内不变的标识符。 / Symbol interface providing stable identification. Symbols implementing this
...
/**
 * 可标识符号接双 / Identifiable Symbol Interface
 *
 * 提供符号标识能力的接口，实现此接口的符号具有唯一标识符。 / Interface providing symbol identification capability.
...
/**
 * 拥有者符号基类接双 / Owned Symbol Base Interface
 *
 * 继承臌[Symbol] 接口，为符号添加唯一标识符属性。 / Extends the [Symbol] interface, adding unique identifier property to symbols.
 *
...
/**
 * 拥有者符号数据类 / Owned Symbol Data Class
 *
 * 具有唯一标识符的符号实现，用于在符号计算中区分相同名称的不同符号实例。 / Symbol implementation with unique identifier,
...
data class OwnedSymbol(
    ...
    /**
     * 从现有符号创建拥有者符号。 / Creates an owned symbol from an existing symbol.
     *
...
/**
 * 获取符号的稳定标识符。 / Gets the stable identifier of a symbol.
 *
 * 根据符号类型返回其稳定标识：
...
/**
 * 检查符号是否具有稳定标识。 / Checks whether the symbol has a stable identifier.
 *
...
/**
 * 获取符号的稳定标识符。 / Gets the stable identifier of the symbol.
 *
...
/**
 * 获取或生成符号的稳定标识符。 / Gets or generates the stable identifier of the symbol.
 *
 * 如果符号实现了 [StableSymbol]，返回其稳定标识；否则使用名称和对象哈希码生成。 / If the symbol implements [StableSymbol], returns its stable ID; otherwise generates one
...
/**
 * 创建符号的拥有者副本。 / Creates an owned copy of the symbol.
 *
 * @param id 符号标识符，默认使用符号的稳定标诌/ Symbol identifier, defaults to the symbol's stable ID
...
/**
 * 获取符号的身份字符串。 / Gets the identity string of a symbol.
 *
...
/**
 * 默认符号比较噌 / Default Symbol Comparator
 *
 * 用于符号排序的比较器，首先按名称比较，名称相同则按身份标识比较。 / Comparator for sorting symbols, first by name, then by identity if names are equal.
*/
...
/**
 * 默认稳定符号比较器 / Default Stable Symbol Comparator
 *
 * 用于符号排序的比较器，首先按名称比较，名称相同则按稳定标识比较。
 * 要求符号具有显式的稳定标识，否则抛出异常。 / Comparator for sorting symbols, first by name, then by stable identifier if names are equal.
