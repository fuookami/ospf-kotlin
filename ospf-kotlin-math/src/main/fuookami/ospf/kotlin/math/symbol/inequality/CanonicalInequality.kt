/**
 * 规范不等弌 / Canonical Inequality
 *
 * 定义规范形式的不等式，左右两边均为规范多项式。
 * 规范不等式支持任意次数的多项式，是最通用的不等式表示形式。 / Defines inequalities in canonical form, where both sides are canonical polynomials.
 ...
 * 规范不等式可以表示任意次数的多项式不等式，是最通用的不等式表示形式。 / Represents an inequality in canonical form, containing left-hand polynomial,
