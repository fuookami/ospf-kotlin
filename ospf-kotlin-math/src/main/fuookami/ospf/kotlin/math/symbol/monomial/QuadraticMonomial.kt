 ...
 * Quadratic Monomial
 *
 * 表示二次单项式，形如 c*x*y 或 c*x²，其中 c 为系数，x、y 为符号变量。
 * 当 symbol2 为 null 时，表示线性项 c*symbol1；当 symbol1 == symbol2 时，表示纯二次项 c*symbol1²。
 * 二次单项式是构建二次多项式的基本单元。
 * Represents a quadratic monomial of the form c*x*y or c*x², where c is the coefficient
 * and x, y are symbol variables. When symbol2 is null, it represents a linear term c*symbol1;
 * when symbol1 == symbol2, it represents a pure quadratic term c*symbol1².
