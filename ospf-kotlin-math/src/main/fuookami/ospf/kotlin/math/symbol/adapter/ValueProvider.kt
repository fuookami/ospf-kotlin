 ...
 *
 * 函数式接口，用于根据符号查找对应的浮点数值。
 * 在表达式求值时，通过此接口获取符号绑定的实际数值。
 * Functional interface for looking up floating-point values by symbol.
 * During expression evaluation, retrieves the actual numeric values bound to symbols.
 *
 * @see MapValueProvider 基于 Map 的实现 / Map-based implementation
 */
fun interface ValueProvider {
    /**
     * 获取符号对应的值
     * Gets the value for a symbol
     *
 ...
 * Map Value Provider
 *
 * 基于 Map 的值提供者实现，使用 Map 存储符号到数值的映射关系。
 * Map-based value provider implementation, storing symbol-to-value mappings in a Map.
 *
...
class MapValueProvider(
    ...
    /**
     * 从内部 Map 中获取符号对应的值
     * Gets the value for a symbol from the internal Map
     *
     * @param symbol 要查找值的符号 / The symbol to look up the value for
     * @return 符号对应的浮点数值，如果不存在则返回null / The floating-point value for the symbol, or null if not found
     */
 ...
 *
 * 定义当值提供者找不到符号对应值时的处理策略。
 * Defines the handling strategy when a value provider cannot find a value for a symbol.
 *
 * - [ReturnNull]: 返回 null，表示无法获取值 / Returns null to indicate the value cannot be retrieved
 * - [AsZero]: 将缺失值视为零 / Treats missing values as zero
 * - [Fail]: 抛出异常，表示这是错误情况 / Throws an exception to indicate an error condition
 */
enum class MissingValuePolicy {
    /**
     * 返回 null，表示无法获取值
     * Returns null to indicate the value cannot be retrieved
     */
    ReturnNull,
    ...
    /**
     * 抛出异常，表示这是错误情况
     * Throws an exception to indicate an error condition
     */
    Fail
}
