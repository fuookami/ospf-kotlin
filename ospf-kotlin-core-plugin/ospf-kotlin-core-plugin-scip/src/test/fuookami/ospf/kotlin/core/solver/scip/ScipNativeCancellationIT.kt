/** SCIP 原生取消集成测试。 / SCIP native cancellation integration tests. */
...
/** SCIP 原生中断必须转换为结构化 Cancelled 报告。 / SCIP native interruption must become a structured Cancelled report. */
class ScipNativeCancellationIT {
    @Test
    fun nativeMipCancellationReturnsCancelledReport() = runBlocking {
        ...
        val report = requireAvailable(result, "SCIP native cancellation")
    ...
    private fun cancellationModel(): LinearTriadModel {
        ...
        return LinearTriadModel(
            impl = BasicLinearTriadModel(
                ...
                name = "scip-native-cancellation-it"
            ),
