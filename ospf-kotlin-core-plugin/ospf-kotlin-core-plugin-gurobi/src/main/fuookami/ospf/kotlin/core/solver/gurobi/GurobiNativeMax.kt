...
/** 使用 Gurobi 10 数值边界准备 MAX 数据。 / Prepare MAX data with Gurobi 10 numeric limits. */
...
/** 隔离 Gurobi 10 MAX SDK 写入。 / Isolate Gurobi 10 MAX SDK writes. */
    ...
    for (data in prepared) {
        try {
            ...
            return Failed(
                ...
                "Gurobi MAX API 不可用：${error.message ?: error::class.simpleName} / " +
                    "Gurobi MAX API is unavailable: ${error.message ?: error::class.simpleName}"
            )
        } catch (error: Exception) {
            return Failed(
                ErrorCode.OREngineModelingException,
                "Gurobi MAX 写入失败：${error.message ?: error::class.simpleName} / " +
                    "Gurobi MAX write failed: ${error.message ?: error::class.simpleName}"
            )
        }
    }
    return ok
...
/** 使用 Gurobi 10 原生 MAX；仿射输入使用 SDK-only 连续变量。 / Write Gurobi 10 native MAX with SDK-only affine variables. */
...
private fun validateGurobiNativeMaxData(
    ...
    if (data.resultKey !in variableKeys) {
        return Failed(
            ...
            "Gurobi MAX 结果变量缺失：${data.name} / Missing Gurobi MAX result variable: ${data.name}"
        )
    }
    if (data.selectorKeys.any { it in variableKeys }) {
        return Failed(
            ErrorCode.IllegalArgument,
            "Gurobi MAX selector 未被省略：${data.name} / Gurobi MAX selector was not omitted: ${data.name}"
        )
    }
    if (data.inputs.any { input -> input.terms.keys.any { it !in variableKeys } }) {
        return Failed(
            ErrorCode.IllegalArgument,
            "Gurobi MAX 输入变量缺失：${data.name} / Missing Gurobi MAX input variable: ${data.name}"
        )
    }
    if (data.inputs.any { input ->
        ...
        return Failed(
            ErrorCode.IllegalArgument,
            "Gurobi MAX 数值不可用：${data.name} / Invalid Gurobi MAX numeric data: ${data.name}"
        )
    }
    return ok
}
