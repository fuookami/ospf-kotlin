...
class GurobiNativeAbsTest {
    private val input = RealVar("gurobi_abs_input")
    private val result = URealVar("gurobi_abs_result")
    private val positive = URealVar("gurobi_abs_positive")
    private val negative = URealVar("gurobi_abs_negative")
    private val sign = BinVar("gurobi_abs_sign")
    ...
    fun capabilityRequiresExactAbsSignature() {
        assertTrue(gurobiFunctionSolverCapabilities().supports(
            FunctionNativeCapability.GeneralAbs
        ))
        assertFalse(gurobiFunctionSolverCapabilities(WrongSignature::class.java).supports(
            FunctionNativeCapability.GeneralAbs
        ))
    }
