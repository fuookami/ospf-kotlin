...
/** Gurobi 10 原生 MAX 端到端集成测试。 / Gurobi 10 native MAX end-to-end integration tests. */
class GurobiDeferredMaxIT {
    @Test
    fun nativeAffineMaxSupportsSlackRangeWithoutExportingSdkHelpers() = runBlocking {
        ...
        try {
            ...
            val report = requireAvailable(
                ...
                "Gurobi native affine MAX through SlackRange"
            )
    ...
    fun nativeMaxRestoresMixedAbsAndPiecewiseResults() = runBlocking {
        ...
        try {
            ...
            val report = requireAvailable(
                ...
                "Gurobi mixed native ABS PWL MAX"
            )
    ...
    fun partialMaxWriterFailureRebuildsTheWholeModel() = runBlocking {
        ...
        try {
            ...
            val report = requireAvailable(
                ...
                "Gurobi fallback after partial MAX writer failure"
            )
    ...
    private suspend fun buildSlackRangeScenario(): SlackRangeScenario {
        val input = RealVar("gurobi_max_slack_input")
        ...
        val function = SlackRangeFunction(
            ...
            name = "gurobi_max_slack"
        )
        val metaModel = LinearMetaModel<Flt64>(
            name = "gurobi-max-slack-it",
        ...
        return try {
            ...
            assertTrue(metaModel.addConstraint(pointConstraint(input, Flt64(3.0), "gurobi_max_slack_point")) is Ok)
    ...
    private suspend fun buildMixedScenario(): MixedScenario {
        val absInput = RealVar("gurobi_max_mixed_abs_input")
        val pwlInput = RealVar("gurobi_max_mixed_pwl_input")
        val maxFirst = RealVar("gurobi_max_mixed_first")
        val maxSecond = RealVar("gurobi_max_mixed_second")
        ...
        val maxFunction = MaxFunction(
            ...
            bigM = Flt64(4.0),
            converter = IntoValue.Identity,
            name = "gurobi_max_mixed_max"
        )
        val metaModel = LinearMetaModel<Flt64>(
            name = "gurobi-max-mixed-it",
            configuration = MetaModelConfiguration(functionExpansionPolicy = FunctionExpansionPolicy.DEFERRED_NATIVE_FIRST),
            converter = IntoValue.Identity
        )
        return try {
            ...
            assertTrue(metaModel.addConstraint(pointConstraint(absInput, Flt64(-1.5), "gurobi_max_mixed_abs_point")) is Ok)
            assertTrue(metaModel.addConstraint(pointConstraint(pwlInput, Flt64(1.5), "gurobi_max_mixed_pwl_point")) is Ok)
            assertTrue(metaModel.addConstraint(pointConstraint(maxFirst, Flt64(-1.0), "gurobi_max_mixed_first_point")) is Ok)
            assertTrue(metaModel.addConstraint(pointConstraint(maxSecond, Flt64(-0.5), "gurobi_max_mixed_second_point")) is Ok)
    ...
    private suspend fun buildHelperFallbackScenario(): HelperFallbackScenario {
        val first = RealVar("gurobi_max_helper_first")
        val second = RealVar("gurobi_max_helper_second")
        first.range.geq(Flt64.zero)
        first.range.leq(Flt64(4.0))
        second.range.geq(Flt64.zero)
        second.range.leq(Flt64(4.0))
        val function = maxFunction(first, second, "gurobi_max_helper")
        val metaModel = LinearMetaModel<Flt64>(
            name = "gurobi-max-helper-fallback-it",
            configuration = MetaModelConfiguration(functionExpansionPolicy = FunctionExpansionPolicy.DEFERRED_NATIVE_FIRST),
            converter = IntoValue.Identity
        )
        return try {
            ...
            assertTrue(metaModel.add(LinearFunctionSymbolAdapter(function, IntoValue.Identity)) is Ok)
            assertTrue(metaModel.addConstraint(pointConstraint(first, Flt64(2.0), "gurobi_max_helper_first_point")) is Ok)
            assertTrue(metaModel.addConstraint(pointConstraint(second, Flt64(3.0), "gurobi_max_helper_second_point")) is Ok)
            assertTrue(metaModel.addConstraint(
                LinearInequality(
                    ...
                    name = "gurobi_max_helper_restriction"
                ),
                name = "gurobi_max_helper_restriction"
            ) is Ok)
    ...
    private suspend fun buildTwoMaxScenario(): TwoMaxScenario {
        val firstInput = RealVar("gurobi_max_failure_first_input")
        val secondInput = RealVar("gurobi_max_failure_second_input")
        val thirdInput = RealVar("gurobi_max_failure_third_input")
        val fourthInput = RealVar("gurobi_max_failure_fourth_input")
        listOf(firstInput, secondInput, thirdInput, fourthInput).forEach {
            it.range.geq(Flt64.zero)
            it.range.leq(Flt64(4.0))
        }
        val first = maxFunction(firstInput, secondInput, "gurobi_max_failure_first")
        val second = maxFunction(thirdInput, fourthInput, "gurobi_max_failure_second")
        val metaModel = LinearMetaModel<Flt64>(
            name = "gurobi-max-writer-failure-it",
            configuration = MetaModelConfiguration(functionExpansionPolicy = FunctionExpansionPolicy.DEFERRED_NATIVE_FIRST),
            converter = IntoValue.Identity
        )
        return try {
            ...
            assertTrue(metaModel.addConstraint(pointConstraint(firstInput, Flt64.one, "gurobi_max_failure_first_point")) is Ok)
            assertTrue(metaModel.addConstraint(pointConstraint(secondInput, Flt64(2.0), "gurobi_max_failure_second_point")) is Ok)
            assertTrue(metaModel.addConstraint(pointConstraint(thirdInput, Flt64(3.0), "gurobi_max_failure_third_point")) is Ok)
            assertTrue(metaModel.addConstraint(pointConstraint(fourthInput, Flt64.one, "gurobi_max_failure_fourth_point")) is Ok)
    ...
    private suspend fun requireMechanism(metaModel: LinearMetaModel<Flt64>): LinearMechanismModel<Flt64> {
        return when (val result = LinearMechanismModel.invoke<Flt64>(
            ...
            is Ok -> result.value
            is Failed -> fail(result.error.message ?: "failed to build Gurobi MAX mechanism model")
