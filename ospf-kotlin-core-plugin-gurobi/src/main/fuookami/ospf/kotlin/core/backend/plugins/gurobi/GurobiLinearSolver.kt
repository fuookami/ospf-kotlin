...
private class GurobiLinearSolverImpl(
    ...
    operator fun invoke(model: fuookami.ospf.kotlin.core.backend.intermediate_model.LinearTriadModel): Result<LinearSolverOutput, Err> {
        ...
        val gurobiConfig = if (config.extraConfig is GurobiSolverConfig) { config.extraConfig as GurobiSolverConfig } else { null }
        val server = gurobiConfig?.server
        val password = gurobiConfig?.password
        val connectionTime = gurobiConfig?.connectionTime

        val processes = arrayOf(
            { if (server != null && password != null && connectionTime != null) { it.init(server, password, connectionTime) } else { it.init() } },
            { it.dump(model) },
    ...
    private fun dump(model: fuookami.ospf.kotlin.core.backend.intermediate_model.LinearTriadModel): Try<Err> {
        return try {
            ...
            while (i != model.constraints.size) {
                ...
                grbModel.addConstr(lhs, GurobiConstraintSign(model.constraints.signs[i]).toGurobiConstraintSign(),
            ...
            grbModel.setObjective(obj, when (model.objective.category) {
                ObjectCategory.Minimum -> { GRB.MINIMIZE }
                ObjectCategory.Maximum -> { GRB.MAXIMIZE }
            })
        ...
        return try {
            if (status.succeeded()) {
                ...
                output = LinearSolverOutput(
                    ...
                    Flt64(if (grbModel.get(GRB.IntAttr.IsMIP) != 0) { grbModel.get(GRB.DoubleAttr.MIPGap) } else { 0.0 })
                )
            } else {
                // to do
            }
