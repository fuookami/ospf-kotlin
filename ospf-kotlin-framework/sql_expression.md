...
8. README 已在 P7 更新函数表达式边界；P8 实现过程中只更新 `sql_expression.md` 当前状态，等 P8/P9 完成后再整体更新属性引用与 schema block 推荐示例。
...
abstract class PredicateSchema<E> {
    ...
    fun predicate(block: PredicateSchema<E>.() -> BooleanExpression): BooleanExpression =
        block()
}
```
...
1. expression DSL
   - 新增 `TypedPathBuilder<E, T>`。
   - 新增 `prop(KProperty1<E, T>)`。
   - 新增 `PredicateSchema<E>` 手写 schema block。
   - 增加 typed `infix eq/ne/lt/le/gt/ge`、点调用 `eq/ne/lt/le/gt/ge`、列-列比较、`inValues/notInValues`、null check、字符串 pattern match。
   - 保持现有 `path("...")` API 兼容。

2. framework expression
   - 为 `SortBy` 增加 `KProperty1` 重载。
   - 为 `UpdateAssignments` 增加 `KProperty1` 重载。
   - 不改变现有字符串 path 构造 API。

3. resolver 辅助
   - 可新增基于属性名的简单 resolver helper，例如 `propertyNameResolver(vararg properties)`。
   - 不强制引入数据库列注解解析；注解/命名策略可作为后续 P9 或后端专属能力。

4. 文档策略
   - P8 实现过程中只更新 `sql_expression.md` 当前状态。
   - P8/P9 均完成后，再整体更新 `README.md` / `README_ch.md`，将推荐示例从 `path("status")` 调整为 schema block 与 `prop(User::status)`，并说明字符串路径是公共 AST/fallback。
...
1. 业务侧可以不写字段字符串，使用 `KProperty1` 构造常见 predicate、sort、update。
2. 生成的 AST 仍只包含 `PropertyPath` / `ScalarReference`，不携带不可序列化的 `KProperty1`。
3. 类型约束生效：`prop(User::age).gt(18)` 可用，`prop(User::age).gt("18")` 在编译期不可用。
...
3. P7 函数表达式跨层复用已完成。
4. P8 KProperty 强类型属性引用 DSL、P9 PredicateSchema 代码生成是下一轮目标。
