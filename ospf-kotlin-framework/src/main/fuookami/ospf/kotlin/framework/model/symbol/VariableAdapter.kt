...
/**
 * 变量适配器，用于建立内部变量与外部变量之间的映射关系。 / Variable adapter that establishes a mapping relationship between an inner variable and an outer variable.
...
data class VariableAdapter<Type : VariableType<*>>(
    ...
    /**
     * 内部变量，表示适配后的变量包装。 / Internal variable representing the wrapped variable after adaptation.
    ...
    */
    ...
    /**
     * 适配器变量，表示适配结果的另一种变量包装。 / Adapter variable representing an alternative variable wrapping of the adaptation result.
    ...
    */
    ...
    /**
     * 将当前变量适配为内部变量，使用给定的适配器。 / Adopt the current variable as an internal variable using the given adapter.
    ...
    */
    ...
    /**
     * 将外部变量包装为适配器变量。 / Wraps the outer variable as an adapter variable.
     *
...
/**
 * 从适配器列表中查找当前变量对应的适配器。 / Find the adapter corresponding to the current variable from the given adapters.
