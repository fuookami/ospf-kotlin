 ...
 *
 * 提供强类型列绑定能力，作为现有字符串 resolver 的补充选项。 / Provides strong-typed column binding capability as a complement to the existing string resolver.
 *
 * 设计说明 / Design Notes:
...
/**
 * 列映射接口（KSP 生成的 schema 实现此接口） / Column mapping interface (KSP generated schema implements this)
*/
...
interface ColumnBinder<C> {
    ...
    /**
     * 解析属性路径为列 / Resolve property path to column
     *
...
*/
fun <C> ColumnBinder<C>.toResolver(): PersistenceFieldResolver<C> = PersistenceFieldResolver { path -> resolve(path) }
