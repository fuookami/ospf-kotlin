...
object MySQL {
    ...
    operator fun invoke(config: MySQLConfig): Database? {
        ...
        return try {
            val client = Database.connect("jdbc:mysql://${config.userName}:${config.password}@${config.url}/${config.database}?charset=utf8mb4")
    ...
    operator fun invoke(name: String, dataBase: String? = null): Database? {
        return if (dataBase != null) { clients[MySQLClientKey(name = name, database = dataBase)] } else { null }
